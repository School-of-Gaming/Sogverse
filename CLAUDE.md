...
**Rule: Never use hardcoded colors or raw Tailwind color classes (e.g. `text-sky-400`, `bg-red-500`).** All colors must come from CSS custom properties defined in `src/app/globals.css` and referenced via semantic Tailwind classes (`text-primary`, `bg-destructive`, etc.). For non-CSS contexts (email templates, canvas), use the hex constants in `src/lib/constants/colors.ts`. This ensures a single source of truth for colors and brand identity. **The one carve-out is artwork**, which carries its own hex palette in its own constants and never references a brand token at all — a pixel-art trophy's gold is a trophy's gold, not the brand's amber — which is the same doctrine the shading rule's art exemption below states.

**Rule: a brand colour exists at exactly the values it was authored at — a family's strong or soft variant, or a token's own full value — and never at an alpha step.** `bg-primary/10` does not draw a lighter amber: over the near-black ground it composites to a darker, duller one, so what the reader sees is no longer the brand. A ground that needs to lift goes to a neutral (`bg-muted`, `bg-accent`) and the brand arrives at full value on the edge, the ink or the fill. Three exemptions, and the list is closed: **chip-scale icon-accent tiles** (a tint lighting a glyph is not a colour painted as a card's ground — the card behind it stays neutral, which is the constraint the exemption came with); **the two sanctioned home keeps** — the hero's amber→violet band, shared verbatim by `/roblox`, and the closing CTA's wash — pre-existing identity moments kept as they are, not a licence for new ones; and **artwork**, which is outside the rule entirely because it never references a brand token at all (a pixel-art trophy's gold is a trophy's gold, and it carries its own hex constants). A unit test regenerates the census and fails on anything outside that list, because prose decays and a failing test does not.

**Rule: colour is grammar — one family, one meaning, everywhere.** Amber = **act** (CTAs, links, the mark); harmony pink = **people**; glow green = **growth** (progress, achievement, presence, liveness); wit blue = **knowledge** (and time ahead); valor orange = **adventure** (camps, events); violet = **the world** — lore, display, identity — and nothing else, which is what stopped it meaning five unrelated things. A hue that means one thing on one page and another on the next teaches the reader to ignore it. Two riders: a **grammar-coloured button** only where the action *is* the family's word, never beside a primary CTA and never two in one view (amber keeps the act monopoly); and **mechanical acknowledgements stay success-green** — copied, saved, sent — by owner ruling, because green is the affirmative register and muted reads dismissive, even though it costs the ambition of hearing glow least. Where colour is genuinely free — decoration, identity, marketing — spend it on the family the surface hears least.

**Corollary: a fact owns its tone, decided once in a shared map beside the fact — never per surface.** A surface that picks its own colour for a fact every other surface also draws is how one meaning acquires three, and the census found exactly that: "live" rendering in two colours across four sites, a missing gedu fee muted on one page and destructive on the next. Reach for the map; if a surface genuinely needs to differ, the reason belongs beside the class.

**Corollary: brand-hue gradients are retired by default.** Blending two families smears two committed meanings into a third that means nothing — "we have a lot of colours now, we don't need to smear anymore". The exceptions are the two sanctioned keeps named above, and a new gradient needs a case made for that site specifically. A same-hue fade to transparent is a wash, not a smear, and is governed by the shading rule instead; a gradient travelling only between one family's own authored values (the live card's ignition ring) is compliant because no pixel leaves the palette's line.

**Rule: the strength axis is what a family's colour is *doing*, and there are three answers.** **Act** is a solid fill with dark ink. **Label** is `bg-muted` under the family's ink — the tone is carried by the text, not by a tinted ground. **Selection** is a brand edge over a neutral lift (`border-primary bg-accent`): the selected thing has to read as selected *as a whole*, and it cannot get that from a shaded brand ground, so the lift comes from a neutral token while the amber stays at full value on the edge. **Which variant a use takes is settled per family by contrast, and wit's text and ink always take soft** — wit-strong measures 3.31–4.10:1 across this app's three grounds, clearing 3:1 for a glyph and missing 4.5:1 for body copy, so strong stays for fills, edges and swatches that carry no body text; `--info` *is* wit-strong, so info ink follows the same mechanism and an info-toned label spells `text-yty-wit-soft` while `border-info` and `bg-info` stand.

**Rule: furniture edges are neutral, and a coloured border is one that has a job.** `border-border` / `border-input` for card edges at rest, rows, add affordances and every other structural line; colour only where the border *is* the construct — the selection edge, the Live chip, the ignition ring on a live card, a voice zone's own-colour border, form validation's `border-destructive`, a status alert's full-value family edge on a muted ground, an outline chip that has no ground to carry its tone, the Yty accent tile. **The chip split's "with a ground → neutral edge" was written when a chip's ground could be tonal, and the tint ban left no such ground: on the neutral `bg-muted` ground every chip now sits on, a chip carrying a status or a family wears its full-value family edge** — which is what the landed corpus already draws, from the live and future tags to the attendance marks to the alert law's own "full-value family edges on muted grounds". **A border that carries state is never repainted by a hover**: an amber hover landing on a live card's green state edge is two facts fighting over one line, and the hover loses — state-bearing cards take the neutral gray lift instead.

**Rule: colour earns its place at rest.** A border, chip or mark that is only coloured on hover is vibrancy spent where a phone cannot see it, and parents meet this product on phones — so where a thing deserves colour, it wears it at rest, and hover stays functional feedback in the neutral idiom (an accent lift, a shadow, a ring). Desktop-default admin and gedu surfaces may still spend colour on hover where it earns its keep.

**Rule: the colour budget follows the audience — parent, gamer and gedu surfaces lean colourful; admin leans restrained.** The families and the gedus running their sessions are who the palette is *for*; an admin is reading dense rows of state all day, and a page that shouts everywhere says nothing. This is a site-wide lean, not a per-page judgment call.

**Rule: each role has a colour, and the app reinforces it wherever a role is understood — even with no role label in sight.** Gamer amber, parent harmony-strong, gedu wit-soft, admin the app's own ink (`bg-foreground`). Role is the one categorical fact that recurs on every surface, so a reader who learns it once reads it everywhere; a badge is where it is *stated*, not the only place it may appear.

**Rule: Poppins is the app face — body copy and every heading, with no display face beside it — and every face is loaded through `next/font`.** There is no third answer for a heading that wants personality: the pinned scale (H1 48–56px / 600 / 1.1, with 30px as the mobile step; H2 36 / 600 / 1.2; H3 24–28 / 600 / 1.3) is what a big heading gets. Space Mono is a sanctioned brand face loaded the same way, and it is spent on exactly two placements — the voice room's zone names and `/select-profile`'s "Who is entering Sogverse?" — because both are the platform naming one of its own places. That list is closed: marketing copy, user content, badges, section headings and the dashboard section pill are all Poppins by ruling, and a third placement is a decision to raise rather than a class to apply. The one other sanctioned face is **Dancing Script (`--font-cursive`), for signature lines and nothing else** — a name typed into a signing field, rendered the way it would be signed (the gedu contract flow and the certification card); anything longer than a name is unreadable in it, which is what keeps the scope closed rather than merely narrow (owner exception, 2026-09-01, logged in the deviations doc).
