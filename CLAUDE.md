...
- **the Sogverse** — a third sense, and a settled one: with the definite article, the name of the single connected universe our Minecraft servers share and of the stories told inside it. This is the sense the Yty lore copy uses, and that copy keeps the word. A reader meeting "the Sogverse" there is being handed the name of a place in a fiction, not the platform's name cold, so nothing below about introducing the platform name applies to it. The article is the tell: the platform takes none.
...
This is a deliberate, in-progress shift, and most of it has now landed. Three parts are finished:
...
One part is still open, and should not be read as settled:

- **How a possessive names the thing you have an account on** — "your Sogverse account" or "your School of Gaming account". The codebase currently says both: the email-verification mail asks about "your School of Gaming account" while the password- and PIN-reset mails say "your Sogverse password" and "your Sogverse parent account", and the auth pages' descriptions ("Sign in to your Sogverse account", "Create your Sogverse parent account", "Set a new password for your Sogverse account") follow the platform. Neither side is obviously wrong — the account is a thing the platform holds, and it is also a relationship with the company — which is why the split survived a sweep that fixed the cold uses around it. Decide it once for all of them rather than per string; until then, leave those strings alone and do not read either half as the precedent.

Everything else still naming the platform in `messages/` is prose the corollaries above already decide — a legal page defining who runs Sogverse, a switcher asking who is entering it, the lore's "the Sogverse". Weigh each against the subject of its own sentence as you touch it, rather than sweeping.
