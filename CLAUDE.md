...
- **A procedure run against live systems is a skill in `.claude/skills/`, never a doc** — creating accounts, correcting data, erasing a user, investigating prod, reading analytics, Stripe and Vercel operations. A skill's description is in context in every session, so it is found without a pointer; `.claude/skills/CLAUDE.md` carries how to write one, and a changed procedure updates its skill in the same change.
