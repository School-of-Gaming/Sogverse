...
This is a deliberate, in-progress shift. Two parts of it are finished: the *lockups* — every string carrying both names leads with the brand — and the **cold uses in `messages/`**, which have been swept. Everything a stranger could meet with the platform word un-introduced now says the brand instead: the public help page's opening line and its contact card, the public Gedu registration title, and the transactional-email copyright footer, which names the company that holds the copyright and so matches the site footer exactly. Two parts are still open, and neither should be read as settled:

- **The sub-page title template and `og:site_name`** (`src/app/layout.tsx`) are platform-only: every page but the root renders as `<page> | Sogverse`, and a shared link names Sogverse alone. That is more cold-contact title surface than the root title this rule already governs, and the pages most likely to be shared cold already opt out of it by hand — a product page builds an absolute `… | School of Gaming` title of its own — so the template is now the minority answer on the surface where it matters most. Changing it is still an SEO and information-architecture decision, not a typo fix, which is why it is flagged rather than quietly done.
- **Prose the rule does not decide on its own.** What is left in `messages/` is the genuinely two-sided kind: a sign-in screen greeting somebody who by definition already has an account, the Yty lore pages where "the Sogverse" is the name of a fictional universe rather than a product, and the legal pages, where "Sogverse is run by School of Gaming Galactic Oy" is a definition doing exactly the work a definition should. Weigh each as you touch it rather than sweeping, and don't read an un-migrated one as the precedent.
