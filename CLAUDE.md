...
**Until 2026-10-30, also keep the explicit `REVOKE` on non-public functions** (`REVOKE EXECUTE ... FROM PUBLIC, anon, authenticated`). Hosted DBs retain the legacy auto-expose default privileges until then, so a newly created function gets per-object grants to every Data API role at creation — grants that `REVOKE ... FROM PUBLIC` alone does not remove. CI cannot catch this class: fresh stacks never see the hosted auto-grant, so the function looks locked down locally while staging/prod expose it (this bit `00097`/`00098` — the divergence only surfaced in the `schema.sql` dump diff). After 2026-10-30 the REVOKEs become harmless no-ops and this paragraph can be deleted.
