...
**Rule: Password changes go through the emailed reset flow — never a direct `updateUser({ password })` on an ordinary session.** Auth is configured with `security_update_password_require_reauthentication = true`, and GoTrue keys that gate on the age of the *session row* (its `created_at`), not on token freshness — refreshing tokens never clears it. A direct update therefore works in fresh-session testing and fails with `reauthentication_needed` for any user signed in longer than the window, which in practice is most of them. The one supported path is the emailed single-use link completed on `/reset-password` (the Settings "Reset password" button and `/forgot-password` both just send that email), whose `verifyOtp` session passes the gate by construction. Two facts to design around: a completed reset deletes every *other* session (other devices die at their next token refresh, after up to an hour of already-issued-JWT grace), and gamers have no password at all — they sign in via parent account-switch, and must never be given a way to set one, because a typed credential would be a login path around parental control.
