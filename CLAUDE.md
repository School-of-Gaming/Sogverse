...
**Rule: Anything with a time of day renders in the *viewer's* timezone — never the runtime default, never the source/product zone.** A true instant (`*_at` columns) or a date+time (a session, an event, a recurring slot) is shown in the viewer's IANA zone: client components read it from `useTimezone()` and a stable clock from `useNow()`; server components/helpers from `await getServerTimezone()`. Pass it through `formatDate(date, locale, { timeZone })` / `formatTime(date, locale, timeZone)` / `formatSessionDateTimeRange(...)` — `formatDate` and `formatTime` now *require* `timeZone` at the type level, so the compiler rejects any call that omits it (a genuinely zoneless date uses `formatDateOnly` instead). When the displayed zone differs from the source (products are authored in `Europe/Helsinki`), surface the viewer's short tz abbrev next to the time so the adjustment is visible — the abbrev is already locale-formatted by `Intl`, so it is not a translated string. Recurring wall-clock slots can't be converted without a concrete date (the offset is DST-dependent): resolve one with `nextOccurrenceInstant` (`@/lib/schedule-occurrence`), then derive the weekday + clock face and **re-group in viewer space** (a Helsinki Mon/Wed pair can shift to different viewer weekdays). Compute end times by adding the duration to the *instant* and re-formatting — never string-add the viewer-local start, or a DST transition inside the session corrupts it.

**Rule: A pure calendar date with no time of day stays UTC-pinned via `formatDateOnly` (`@/lib/utils`) — do not give it the viewer's zone.** A camp's `start_date`/`end_date` range, a club term date, a legal `LAST_UPDATED` — these are zoneless, and re-anchoring them to a viewer's zone shifts them off-by-one. Rule of thumb: **a value with a clock face converts; a bare date does not.** (An event's date *does* shift when it carries a slot time — that's a date+time instant; an event with no time stays date-only.)
