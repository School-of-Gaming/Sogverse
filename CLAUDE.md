...
- `TODO.md` is the running list of cross-cutting work we know we want to come back to. Distinct from `docs/`. **Adding an item requires the owner's explicit approval — never add one on your own judgment.** Discovering something worth tracking (a review finding, a latent defect, an undecided question) means bringing it to the owner as a proposed item with its justification and writing it only once approved; a mention in a work summary is not approval. **When an item is fully done with nothing left to discuss, delete it — don't check it off (`[x]`).** `TODO.md` tracks open work, not a changelog; the record of what was done lives in git history and in the docs/code the work produced. Leave `[ ]`/`[x]` only for partially-done items where the checked sub-points still give context for the open ones.
