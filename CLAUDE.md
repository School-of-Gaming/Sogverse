...
- **Anti-pattern: never step local days or weeks by fixed 24-hour instant arithmetic** (`+ offset * 86_400_000`, `now - 7×24h`). A local day is 24 hours except on the two DST transition days (23 or 25), so instant stepping repeats or skips a calendar date once a year — and only when the *runtime* zone crosses the transition, so UTC CI can't catch it. Iterate calendar dates instead (year-month-day arithmetic never skips or repeats; anchor constructed dates at noon so no zone's shift can move the date) and convert to an instant at the end. Sibling trap, same UTC-CI blindness: `toZonedTime` results carry the zone's wall clock in their **local** fields — reading `getUTC*` off one only agrees on a UTC runtime, and in any non-UTC browser it lands on the wrong weekday once the runtime offset crosses midnight.
...
**Rule: the dividing line between the two surfaces is whether a piece needs the page around it to be judged.** The style guide is for anything that can be looked at on its own; a preview scene is for anything that can only be assessed against what sits near it. Ask what you would be unable to tell from a demo card. If the answer is "nothing" it belongs in the style guide, however large the thing is.

**A dialog belongs in the style guide, not a scene.** It opens in its own overlay above whatever page summoned it, so the page contributes nothing to how it reads — its width, its spacing and its behaviour are entirely its own, and a scene would only be lending it a backdrop nobody is looking at. The same goes for any overlay: a popover, a sheet, a toast. **Content that sits in a page's flow is the opposite case**: a feed, a card in a column, a panel in a rail — these are judged by how they sit against their neighbours, at that page's width, with that page's scrolling, and a demo card that isolates them has thrown away the only thing you needed to see. Those get a scene.
