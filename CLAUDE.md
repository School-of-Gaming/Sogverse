...
**In an emailed button row the *position* carries over; the emphasis deliberately does not.** A mail's two-button row is a fixed 50/50 table that is a row at every width, so there is nothing for `col-reverse` to do and the action being asked for goes in the right-hand cell, reading the way the app has already taught. But the row's own type forbids a filled half — two buttons in a mail are offered as alternatives, and the shape it must not be able to make is two brand-filled cells competing — so both halves are outlined and the right-hand one carries no visual weight the left lacks. Position is the whole of what transfers.
