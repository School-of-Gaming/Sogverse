...
- **This is a deliberate simplification, and re-enabling seat caps is what would undo it.** A pre-payment hold protects a capped product from overselling while parents sit on the Stripe page. **The admin form therefore unlocks caps for municipality clubs only, and locks them for every other type** — a muni club is invoiced off-platform, so its signup validates the cap and writes the `active` row inside the same locked transaction and never goes near Checkout. The bar is about the money, not the product type: any no-charge shape clears it, and a *free event* does. It is nevertheless locked, on a separate and later call — the parent-facing **browse card** cannot render fullness for a non-muni product, so a capped event would fill up and still read as open in the listings. (The open detail panel does show a seat bar for any capped product; the gap is on the surface where a family decides whether to click through at all.) That re-lock is a product decision recorded in `TODO.md`, and lifting it is a shop change before it is a form change. Whoever unlocks caps on a shape that *does* reach Checkout has a different problem and should expect to reintroduce a hold; read this section as the record of what removing it cost.
- **`promote_from_waitlist` has no billing-mode guard, and nothing in the form can reach that today.** Promoting a row writes an `active` seat with no payment step, so on a paid product it would grant a free seat. No form path leads there: caps are muni-only, a waitlist exists only behind a cap, and a muni club cannot become paid. The gap is therefore latent rather than live — it opens the moment caps are unlocked for a type whose billing mode can change under them, which is exactly the event case, so the guard is part of that unlock's cost and not a bug outstanding against today's schema.
