...
**Teardown is webhook-driven.** When the parent cancels in the portal, Stripe fires `customer.subscription.deleted` at period end → `handleSubscriptionDeleted` (`src/app/api/webhooks/stripe/products/route.ts`) looks the row up by `stripe_subscription_id`, reads its `participation_id`, and calls `cancel_participation` to hard-delete the participation (which CASCADE-removes the `family_subscriptions` row). Stripe has already cancelled the sub, so this path never calls Stripe back. The handler is idempotent: a replayed deletion finds no row (already torn down) and returns a clean 200. During the cancel-pending window, `customer.subscription.updated` with `cancel_at_period_end` records `status = 'canceling'` and persists `current_period_end`, which the access checks treat as still-live so the child keeps access until the period actually ends.

**Dashboard reflects the cancel-pending window.** While a sub sits `canceling`, the dashboard Sessions section reads its `current_period_end` and (a) clamps the upcoming-session list to occurrences on or before that instant, so a parent/gamer only ever sees sessions they still have paid access to, and (b) — parent only — marks those remaining cards confirming the cancellation was fulfilled (the gamer just sees fewer cards, no badge). Both signals flow from one money-free, owner-scoped RPC, `get_my_participation_subscription_states` (00093), which the same call also uses for the `past_due` payment-problem badge; the client derives both. The clamp lives in `expandUpcomingSessions` (`src/lib/upcoming-sessions.ts`) and, for an open-ended club, lifts the usual 8-occurrence cap so every remaining paid session shows. Only `canceling` clamps — an `active` sub renews monthly, so its `current_period_end` is not terminal.

The clamp compares **instants**, not dates: `current_period_end` is a precise timestamp (Stripe Unix seconds), so a session is kept iff `sessionStart <= current_period_end`. This deliberately mirrors teardown — a card appears iff the participation will still exist at that session's start — rather than treating the boundary as an end-of-day. The parent badge (`SubscriptionEndingBadge`) is intentionally **session-anchored, not billing-anchored**: it reads "Won't renew" on each remaining card and "Last session" on the final one, and keeps `current_period_end` *off the face* (it's the billing system's number and can fall a few days past the last session). The period-end date appears only in the tooltip, where it explains the gap ("last session Jul 12, access runs through Jul 15") and is dropped when the two coincide.
...
**Preview / mock route.** A fixture-driven preview route under `(public)` renders the body in the parent-eye chrome with no DB calls — never indexed (`metadata.robots = { index: false, follow: false }`), reachable only from `/admin/ui-components`, so designers can eyeball the detail page without seeding data.
...
- **Success redirects to the purchase-confirmation page** (`/shop/confirmation?p=<reservationId>`). It's a server component that fetches the participation (RLS-scoped to the customer) + product detail and renders the order summary, schedule, and "what happens next" — no client loading state, no polling. By the time Stripe redirects, it has already waited on our webhook, and every field shown lives on the reserving row from creation anyway, so the page is correct even if a redirect ever beat the webhook. Free signups `router.push` to the same page after `create_participation`. The page is keyed by participation id and gated by RLS, so it's effectively revisitable for any of the customer's own participations (no one-time/"consumed" flag).
...
- **Dead-end detail panels.** Browse-card CTAs only link to actionable states; the `ended` / `running_late` / `full_closed` states have no normal browse → detail entry, so they collapse to one shared `ClosedPanel` (the state kinds stay distinct for the card layer; only the panel rendering merges). It renders defensively for direct-URL access and in-session state transitions. The UI Components page is the canonical regression surface for it — keep its preview tile current.
