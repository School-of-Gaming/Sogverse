...
- **`topic` is a fixed Postgres enum**, holding a handful of games plus a few non-game subjects; adding one is a migration, not a row, and there is deliberately no admin topic CRUD. The enum stores *only* the value — the game-vs-subject split, the display order, the labels and each game's per-game facts (age rating, where to get it) all live in code (`src/lib/products/topics.ts`), not the DB. Game labels are literals (never translated, they're brand proper nouns); subject labels localize. The parent-facing prose *about* a game is translated even though its name isn't, so a new game topic still needs message entries in every locale. This list is deliberately not enumerated here — it went stale twice; read the module.
