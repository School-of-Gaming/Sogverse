...
- **Session writes are gedu-gated on the group assignment** — any gedu assigned to a group may edit that group's sessions, attendance and notes; admins override everything. Peer-group *feeds* are not readable in v1, and the schema doesn't block opening them: that's a change to one predicate, not a redesign.
...
- **Known wart — a session in progress reads as past.** Entry kind is decided by the *start* instant, so a session that has started and not yet ended sorts into the past and offers its record editor, while the server refuses attendance until the session has actually ended. A gedu who tries mid-session sees the editor's error line; notes save fine, only marks are refused.
- **The family-facing half isn't built.** No parent or gamer surface consumes session reports yet, so `products.padlet_url` and its remaining render sites stay until they do (tracked in `TODO.md`). The product's gedu-only lesson-material link is a *different* field with no backfill from the Padlet — the Padlet held family session notes, the material link is lesson content — and no read path that can reach a family may select it.
