...
- **A session in progress reads as past, on both sides.** Entry kind is decided by the *start* instant, so a session that has started and not yet ended sorts into the past and offers its record editor — and the server accepts its marks from the same instant, so the roll-call flow works while the session is running. The attention count still turns over only at the session's *end* (an unfinished session is not yet owed work), which is the one place the two boundaries deliberately differ.
