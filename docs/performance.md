...
- **Constraint 2 — the fixed pool of 10, which compute does NOT fix.** GoTrue logs `limit_max_open_conns: 10, limit_strategy: "fixed"` and it stayed at 10 across a Small → Large resize even as Postgres `max_connections` went 90 → 160. Its own advisor logged pool starvation on **both** tiers. Raising it is not purchasable; it is set by Supabase's closed-source provisioning.
...
- **Which of the two actually binds: the pool.** Cost per family dropped from 3 bcrypt hashes to 2 when child accounts stopped setting a password (nothing could use it; gamers authenticate by server-minted magic-link OTP). Re-running the identical ramp moved p50 by only **2–15%** (largest at the highest rate) and **did not move the tip-over point at all** — still ~420/min. So hashing is a minority of the auth path's cost and the fixed pool is what caps throughput. **Do not expect further code-level hashing work to raise the ceiling**; the levers that remain are arrival shape, dedicated cores (which cut 504s 4× Small → Large by shortening connection hold times), and asking Supabase to raise the pool.
