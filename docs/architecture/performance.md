...
- *Full win:* scope the session read, the nonce and the preference cookies to the dashboard layer in one PR. Home goes static, TTFB ~400–700ms → ~50ms globally, an edge CDN file. Beats the canonical F2 baseline (`download=660ms` on `/` warm — see the benchmark log).
