...
- Both projects ran free-plan Nano (428 MB) until 2026-08-20. That box could not hold the platform stack resident: chronic swap thrash (~4 TB lifetime) depleted the disk IO burst budget and caused the Aug 19–20 all-services health flap, while the app workload itself measured healthy (64 MB database, 100% cache hit, ~63 s of query time/day). Upgraded four days ahead of the 2026-08-24 Helsinki municipal registration opening (~43 clubs × 15 seats at one instant), which Small then absorbed without a single user-facing failure — see F8, and note what that does and does not prove.
- **Auth rate limits are Supabase stock defaults on prod, and deliberately not on staging.** Prod runs the defaults: `rate_limit_verify` **30 per 5 min per IP** (a token bucket — burst 30, refilling at 360/hour) covering password sign-in, and `rate_limit_token_refresh` 150 per 5 min per IP. Exceeding either is a **429 `over_request_rate_limit`**, which the login form already translates; it is unrelated to the 504s in F7. **Staging's two are pinned at 10000 on purpose** — load generated from a single IP trips a per-IP limit that hundreds of real families never would, and the limit is not what we go there to measure. (Confirmed on prod: the 2026-08-24 opening drew **zero** 429s, so a real cohort genuinely does not share egress enough to collectively trip it. A non-zero count in a future opening still means what F8's watcher says it means — carrier NAT or a school network.) Both are settable via `PATCH /v1/projects/{ref}/config/auth` despite Supabase's docs calling the verify limit non-customisable, and the write takes ~30 s to propagate. Both projects run the same GoTrue build (`GET /auth/v1/health`); check that before assuming a staging measurement transfers to prod.
...
Every protected request — page load, API call, and any RSC prefetch — verifies the caller's identity at three layers:

1. **`src/proxy.ts`** — `supabase.auth.getClaims()` verifies the JWT **locally** against the project's published ES256 JWKS (no GoTrue round-trip). The `getSession()` it calls internally still refreshes a near-expiry token and rotates the cookie, so the proxy stays the single token-refresh point. Then queries `profiles.role` for role-based routing.
2. **The document layout (`src/app/[locale]/layout.tsx`) and `(dashboard)/layout.tsx`** — each reads the session through the shared server-client module (`src/lib/supabase/`): a local `getClaims()` plus a `profiles` query. The two now share **one** query per render via React `cache()` (I2 step 1).
...
`<Link>` does not prefetch: the locale-aware navigation module (`src/i18n/`) sets `prefetch={false}` once, for every call site. A prefetch here cannot deliver anything — with no PPR and no `loading` boundary anywhere in the app, Next short-circuits it to router state alone (no segment data, no head), so there is no cache it can warm that survives to the click, which pays the full dynamic request either way. F1 made each prefetch **cheap** (a local verify); it never made one **useful**. A call site that can use one opts back in explicitly, and the default flips back for any route that gains a static shell. Full reading and the request-count measurements: `docs/investigations/request-amplification.md`.

## Real-user data (Speed Insights)

Vercel Speed Insights collects Core Web Vitals (TTFB/FCP/LCP/INP/CLS plus a Real Experience Score) from real production visitors. Two readers:
...
Append-only, like the benchmark log. Format: `date · window · scope → headline numbers`, with the reading that matters. **Every entry so far predates locale-prefix routing (2026-09-10)** — read them as history, and take a fresh pull before quoting any of them as the current shape.
...
2. **Confirm with Supabase support** whether the 10-connection cap is raisable. **Support is the only route — a compute bump is not one**: the 2026-08-21 rehearsal (F7) resized staging Small → Large and the pool stayed at a `fixed` 10 while Postgres `max_connections` went 90 → 160, with GoTrue's starvation advisor firing on both tiers. Money buys cores, not pool slots.
...
**Residual risk.** Login/signup/token-refresh are *irreducible* GoTrue calls. The free fixes lower probability and add headroom but can't guarantee non-recurrence against a synchronized thundering herd (e.g. a scheduled session start where many users log in within the same second) — that's the one scenario the per-request cleanup doesn't cover. **That scenario has since been driven deliberately and measured: see F7**, which puts numbers on where it breaks and confirms the pool is the half of it no compute tier can relieve.

## Active findings

### F2 — Public marketing pages aren't edge-cacheable

Home and other `(public)` routes load with ~400–700ms TTFB on prod even on fast connections, because they're dynamically rendered per-request instead of served from the edge CDN — proximity to the Stockholm function hides that render tax rather than removing it. That range is the finding's original observation and carries no pull date of its own; the dated real-user figures, and how stale they are, are under *RUM corroboration* below. No single line is "wrong" — the chain from symptom to root cause:

1. The home page is pure static content (translated copy + icons, no DB queries) — it *should* be edge-cacheable.
2. But the document layout (`src/app/[locale]/`) reads the session, reads cookies (timezone, consent) and reads a request header (the proxy-set UTM attribution), while the proxy stamps a per-request CSP nonce. Any one marks the whole subtree dynamic — Next can't pre-render output that depends on the request.
...
4. Locale is not one of the reasons: it lives in the URL, so one URL has one body. Every remaining reason is a `(dashboard)` need sitting above every public page — the session read, the per-request nonce, the preference cookies — plus the UTM header, which genuinely has to stay above everything.

**What blocks a plain `export const dynamic = 'force-static'`:** (a) move the session read out of the document layout into `(dashboard)/layout.tsx`, (b) skip CSP-nonce generation in `src/proxy.ts` for public paths (the per-request nonce makes every response unique HTML), (c) scope the preference-cookie reads similarly, and (d) decide where the UTM-attribution header read lives — it has to stay above every page, and it is a request header, so it cannot simply move to `(dashboard)`. None hard individually — the work is untangling four concerns currently mixed in the document layout, after which a locale-prefixed URL is statically renderable.

**Sequencing:**
- *Small first win, no architecture change:* split layouts so the public tree doesn't do the session read — saves a Supabase round-trip (~50–150ms) per marketing hit without making anything static-eligible. Reversible.
- *Full win:* scope the session read, the nonce and the preference cookies to the dashboard layer in one PR. Home goes static — an edge CDN file, so TTFB stops being a render and becomes a network hop: tens of milliseconds rather than the hundreds F2 records above. The exact figure is unmeasured and deliberately not predicted here; measure it when the work lands. Beats the canonical F2 baseline (`download=660ms` on `/` warm — see the benchmark log) by construction.

**RUM corroboration (2026-08-13 Speed Insights pull) — dated, and not current.** Home TTFB p75 627ms desktop / 331ms mobile — the dynamic-render tax measured on real visitors, matching the 660ms synthetic benchmark, and the strongest real-user evidence for the full F2 fix. `/schools` was the worst public route: TTFB p75 **1886ms, rated POOR** (n=33) — a low-traffic cold-entry page (families arriving from a school's message) pays a cold lambda on top of the dynamic render. The most recent home figure is the 2026-08-28 pull (snapshot log): 489ms desktop / 268ms mobile, improved without F2 being addressed. **Both pulls predate locale-prefix routing (2026-09-10) — calibrate against these numbers, do not re-quote them as today's.** Refresh them with `vercel metrics` for the percentiles, or `npm run perf:insights` for the good/improvable/poor split (the `vercel-analytics` skill records both reads), and append the pull to the snapshot log.

> **Pre-fix number — most of that 1886ms was F6, not F2.** The `/schools` figure was pulled while all three `/schools` routes ran a whole-country municipality read; the controlled probe four days later attributed ~75% of the cold path to that read, which the F6 work (see Completed) removed. Expect this route's number to fall toward the lambda-init + dynamic-render floor rather than to zero — **the dynamic-render tax F2 is about is untouched**, and this bullet's home-page numbers are unaffected by that work and remain the clean F2 evidence.

### F3 — Per-request role lookup is fanned out across request-contexts

On one protected dashboard navigation, `SELECT role FROM profiles` runs in several places: the proxy (`src/proxy.ts`), the layout render (the document layout + `(dashboard)/layout.tsx`, through the shared server-client module), each `requireRole` API call (`src/lib/auth.ts`), plus RLS's `get_user_role()` (`STABLE`, so cached to one call per statement). ~12ms warm each — mostly PostgREST and network latency, not Postgres CPU. **That figure is the one I2 step 2 is gated on, and it carries no pull date, so re-read it rather than re-quote it**: `pg_stat_statements` gives the role query's call count and total exec time as a share of everything (the `supabase-db-inspection` skill covers access).
...
An earlier version of this note claimed every hook pairing server-prefetched `initialData` with a client `useQuery` — the product, participation, family, session and assignment/pin hooks, i.e. all of them — inherits an app default of `staleTime: 0` and so refetches immediately on mount. **That was wrong.** The global default in the query provider (`src/providers/`) is `staleTime: 60 * 1000` (1 minute) — and has been since the initial commit — and no hook overrides it (it's the only `staleTime` in the codebase). React Query treats server-seeded `initialData` as fresh on mount, so within that minute there is **no** immediate refetch; the feared double-fetch doesn't happen.
...
The first Speed Insights pull (see Real-user data) surfaced four routes worth a look. Numbers are 30-day p75s from production RUM — a bullet graduates to its own finding when its cause is established. **All four figures predate locale-prefix routing (2026-09-10); re-pull before working one** (`npm run perf:insights`, or the `vercel-analytics` skill).
...
Measured on staging (resized to match prod) by driving the real API routes at held rates: parent registration → sign-in → PIN → add gamer → claim seat, with anonymous shop browsing underneath. **The database is not the constraint. GoTrue is**, and it is capped two independent ways.

- **The split is stark.** Every step that touches GoTrue costs seconds under load (register 8.1 s, sign-in 15.5 s, gamer create 7.0 s p50); every step that does not costs milliseconds (seat claim 174 ms, PIN 250 ms, waitlist join 125 ms, page render 225 ms). The seat-claim RPC held **zero overbooking, zero duplicates and intact waitlist ordering** with 60 families contending for one 15-seat club — the `FOR UPDATE` product lock does its job. **A family that already has an account is cheap to enrol; creating the account is the whole cost.**
- **Tip-over on Small: ~420 family registrations/min**, where a family is one parent account plus one child account. Comfortable to ~300/min at 1.2 s p50; 360/min → 4 s, 420/min → 12 s, 480/min → 23.5 s. There is a real graceful-degradation band between ~300 and ~420 (slow but succeeding) before hard failures start.
- **Constraint 1 — bcrypt CPU, which compute does fix.** GoTrue runs unthrottled on the same VM as Postgres (no `CPUQuota` in its systemd slice) and its bcrypt cost is hard-coded at 10 with no env binding, so it cannot be tuned. Load average hit **9.77 on Small's 2 shared cores** while RAM and swap never moved. Dedicated cores help proportionally: Large cut `/token` 504s from 49.5% → 12.6% and load to 6.3. Note **Large is still 2 cores** (shared → dedicated); XL is the first tier that adds any.
- **Constraint 2 — the fixed pool of 10, whose *size* compute does not change but whose *throughput* it does.** GoTrue logs `limit_max_open_conns: 10, limit_strategy: "fixed"` and it stayed at 10 across a Small → Large resize even as Postgres `max_connections` went 90 → 160; its own advisor logged starvation on **both** tiers. The number is not purchasable — it is set by Supabase's closed-source provisioning, and GoTrue's newer percentage-based sizing (which *would* scale it with `max_connections`) is not enabled here. **But 10 is generous when the box is healthy**: bcrypt runs *outside* the transaction on both hot paths, so a connection is held only for the insert plus the profile-creating trigger that fires on it — tens of milliseconds, i.e. several hundred auth writes/second across ten connections. It saturates only because CPU starvation stretches those same queries, so each connection is held an order of magnitude too long. **The pool is an amplifier of constraint 1, not an independent wall** — which is why dedicated cores cut 504s 4× without touching the pool size, and why removing a bcrypt hash barely moved anything (it never held a connection to begin with).
- **A 504 here is GoTrue's own 10 s request deadline** (`MaxRequestDuration`), not a gateway timeout, and it does not distinguish waiting-for-CPU from waiting-for-a-pool-slot. **`/api/auth/register` reports it to the parent as "That email could not be registered"** — a transient overload rendered as a permanent rejection, which also steers them to sign in for an account that may not exist. Worth fixing when that route is next touched; a timeout should be a 503 the client can retry.
- **Load leaves orphaned gamer accounts behind.** 11 of ~1,500 child accounts created during the rehearsal ended up with an auth user and a profile but **no `parent_gamer` link** — `gamers/create` creates the auth user before its transactional promotion RPC, and when that RPC fails the compensating `deleteUser` can fail too. Harmless to the family (their gamer list reads through `parent_gamer`, so they simply add the child again) but it accumulates junk rows, and the route's log line is the only trace. **After any high-traffic registration window, sweep for gamer-role profiles with no parent link.** The sweep after the 2026-08-24 opening (F8) found **zero** — but read that as weak evidence: the orphan is produced by the promotion RPC failing under stress, and that opening drove roughly 150 cold registrations rather than the rehearsal's ~1,500, with no failures of any kind. **The defect is untouched, not disproven**; it needs a window that actually strains GoTrue before absence means anything.
- **To check pool state on any project**, search its Auth logs for `applying connection limits to db using the` (gives the literal `limit_max_open_conns` and whether the strategy is `fixed` or `percentage`) and for `Suboptimal database connection pool settings detected!` (GoTrue's own starvation warning, on by default, at most hourly).
- **Removing a hash barely helped, and that is the diagnostic.** Cost per family dropped from 3 bcrypt hashes to 2 when child accounts stopped setting a password (nothing could use it; gamers authenticate by server-minted magic-link OTP). The identical ramp then moved p50 by only **2–15%** and **did not move the tip-over point at all** — still ~420/min. That is consistent with the mechanism above rather than surprising: bcrypt burns CPU but holds no connection, so deleting one shortens no hold time and the pool stays just as saturated. **Do not expect further hashing work to raise the ceiling.** The levers that do move it are, in order: arrival shape, more cores (which shorten the queries that *do* hold connections — the whole reason Large helped), and asking Supabase to raise the pool.

### F8 — What a real opening actually looks like; arrival shape is the lever (measured 2026-08-24)

The 43-club / 645-seat Helsinki municipal opening, watched live on prod at Small. **No parent met a failure: zero GoTrue 504s, zero 429s, and the pooler never had a waiting client, across 549 polls spanning the whole morning.** Data integrity held under real contention — no club overbooked, no duplicate participants, waitlist ordering intact, and the post-window orphan sweep found none.

- **The ceiling was never approached, so this event does not test it.** Peak registrations were **41/min against F7's measured ~420/min tip-over** — about 10% of capacity. Read every comfortable number below as "the expensive path was barely exercised", not as "Small is proven".
- **Why: 73% of participants already had an account** (413 of 563; 150 signed up in-flow). A family that arrives with a session pays page → PIN → claim, all Postgres and milliseconds; only the remainder pay GoTrue. **Pre-registration is not merely a convenience — it converts the one expensive path into the cheap one, and spreads it across the preceding quiet hours.**
- **The arrival curve, which a rehearsal cannot produce.** 302 claims — **47% of all capacity — landed in the first 60 seconds**, and *every one of them* was a pre-registered family. Finer: 26 claims in the first 3 s, 17 in the first 1 s, the first at **213 ms** (human reaction time plus a network hop). The sub-second peak is the 750–1000 ms bucket, not 0–250 ms, because that is what a hover looks like once reflex, click and a round trip are added.
- **Cold families are structurally excluded from the opening seconds.** The first from-scratch registration reached a seat at **65 s**; median register→claim was 70 s (fastest 24 s). Pre-registered claims dominate minutes 0–2, cold claims overtake at **minute 3**, by which time nearly half the seats are gone. Consequence: **pre-registering roughly halved the odds of being waitlisted — 13.3% vs 25.3%.** That is a fairness property, not only a load one.
- **The tail is long and thin.** After minute 10 the rate is single digits per minute and stays there; 77% of seats and 22 of 43 clubs full at 39 minutes. Capacity planning should size the first 60 seconds and ignore the rest.
- **Load spiked to 10.9 on 2 cores — 5.5× saturation — and caused nothing.** Memory dipped 1075 → 500 MB; load was back under 2.0 within ~6 minutes and at 0.27 by minute 39. This is F7's mechanism confirmed from the other side: **the pool starves because CPU starvation stretches the queries holding it, and seat claims hold none.** High load with bcrypt out of the mix is survivable; the number that indicates harm is the 504 count, never load.
- **This was net-new acquisition** — only **3 of 563** participations came from customers older than a week.

**Planning rule this establishes: model an opening by its *pre-registered share*, not by its family count.** The same 800 families are a non-event or a tip-over depending entirely on how many arrive holding a session, and that variable is the one we can actually influence — telling families to make an account before the doors open is simultaneously the fairest and the cheapest intervention available. A cohort with no run-up (short notice, no weekend, a school that did not forward the message) is a materially different load shape from this one and must not be planned against these numbers. The trade the rule makes: a more pre-registered cohort converts GoTrue registrations into signed-in request volume, and that volume lands Vercel-side rather than on the shared database box — PIN-verified sessions skip the proxy's role query — so it is a request-count trade, not an incident-path one (`docs/investigations/request-amplification.md`).
...
### `<Link>` prefetch off by default (branch `feat/prefetch-off-by-default`, 2026-09-21)

**What.** The locale-aware navigation module (`src/i18n/`) wraps the `Link` every call site already imports and sets `prefetch={false}` as the app-wide default, so the framework's viewport prefetch no longer fires once per link on screen. The eight hand-placed `prefetch={false}` props across four components went with it — redundant once the default is off, and there were no `prefetch={true}` opt-ins anywhere to preserve. A call site that can use a prefetch passes one explicitly.

**Why.** A prefetch on this app's shape returns router state and nothing else, so the fan-out was request volume buying nothing — mechanism in the prefetch paragraph under Current infrastructure, full reading in `docs/investigations/request-amplification.md`, which proposed this as its remedy (a).

**Measured before the change** — production, `vercel metrics`, term time: **13–14 function invocations per pageview**, of which **84–86% were prefetch**. Removing them projects a term-time Monday from **13.1 : 1 to roughly 1.8 : 1**.

**Re-measure: pending.** Invocations, proxy runs and pageviews over a month window against that baseline, plus the click-latency control the investigation predicts unchanged. Neither has run, so nothing here is a post-change number — **do not read the projection as a result; pull the real one and append it here.** It is a one-line read: `vercel.request.count` carries an `is_prefetch_request` dimension whose `streaming_func` slice equals function invocations, and Web Analytics supplies the pageview denominator (the `vercel-analytics` skill records both).

**The one mechanism pointing the other way: instance warmth.** Removing ~85% of invocations also removes a trickle that kept function instances warm. Not measured, and judged small — a tail-latency shift for a lone reader at a quiet hour, not a regression class; the argument is in the investigation. **Speed Insights p75 TTFB is the gauge**; no new instrumentation is needed for it.

**Expect the default to flip back per route once a route has a static shell** — a `loading` boundary, or PPR under `cacheComponents` — because then a prefetch buys a paint. That is F2's end state, not separate work.
...
- **Static generation / ISR.** The layout above these routes reads the session on every request, so they are dynamic whatever the page does. That is F2's territory and stays open.
...
**What.** Swapped `supabase.auth.getUser()` (HTTP round-trip to GoTrue) for `supabase.auth.getClaims()` (local ES256 verification against the project's JWKS) in the proxy (`src/proxy.ts`), the RSC layout path (the shared server-client module, `src/lib/supabase/`), and `requireRole` (`src/lib/auth.ts`). Reverted the `prefetch={false}` workarounds (`sidebar.tsx`, `user-row.tsx`, `GroupCard.tsx`, `UpcomingGroupSessionCard.tsx`, `JoinVoiceButton.tsx`) now that per-prefetch auth is cheap. **That revert removed a correct workaround on an incorrect justification** — cheap is not useful, and a prefetch on this app's shape delivers nothing (see the prefetch paragraph under Current infrastructure); `prefetch={false}` is the app-wide default again. The `getClaims` work below is untouched by that and stands as written.
