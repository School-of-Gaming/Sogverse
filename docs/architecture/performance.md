...
2. **The document layout (`src/app/[locale]/layout.tsx`) and `(dashboard)/layout.tsx`** — each calls `getUserWithProfile()` (a local `getClaims()` + a `profiles` query). The two now share **one** query per render via React `cache()` (I2 step 1).
...
An earlier version of this note claimed every hook pairing server-prefetched `initialData` with a client `useQuery` (`useVisibleProductsByTypes`, `useParticipationCounts`, plus `useFamily`, `useMyUpcomingSessionRows`, the assignments/pin hooks) inherits an app default of `staleTime: 0` and so refetches immediately on mount. **That was wrong.** The global default in `src/providers/query-provider.tsx:16` is `staleTime: 60 * 1000` (1 minute) — and has been since the initial commit — and no hook overrides it (it's the only `staleTime` in the codebase). React Query treats server-seeded `initialData` as fresh on mount, so within that minute there is **no** immediate refetch; the feared double-fetch doesn't happen.
...
**Re-measure: pending.** Invocations, proxy runs and pageviews over a month window against that baseline, plus the click-latency control the investigation predicts unchanged. Neither has run, so nothing here is a post-change number — do not read the projection as a result.

**The one mechanism pointing the other way.** Removing ~85% of invocations also removes a trickle that kept function instances warm. It is smaller than it sounds: prefetch volume is proportional to real traffic rather than a background heartbeat — a prefetch fires only because somebody is already on a page — so it never warmed anything through a genuine lull, and in a busy period real traffic keeps instances warm by itself. The residual is a tail-latency shift for a lone reader at a quiet hour who reads for minutes and then clicks. **Speed Insights p75 TTFB is the gauge**; no new instrumentation is needed for it.
