...
2. But `src/app/layout.tsx:52-69` (root layout) calls `getUserWithProfile()`, `getLocale()`, `cookies()`, and `headers()` (the last reads the proxy-set UTM-attribution header). Any one marks the whole subtree dynamic — Next can't pre-render output that depends on the request.
