...
>
> **This is a frozen record of the 2026-03 audit and its remediation.** The *current*
> security posture — the enforcement map, the standing accepted risks (including the
> post-audit `token_hash` query-string item once recorded here as an addendum), and what
> the next audit has to cover — lives in `docs/architecture/security.md`. New findings
> and accepted risks go there, not here.
...
#### CSP gaps at close of remediation

(The *current* accepted-risk status of CSP lives in `docs/architecture/security.md`; the below is the state as recorded at the time.)
...
- **`img-src https:` wildcard — CLOSED (2026-04-15).** Previously required because admins pasted arbitrary HTTPS URLs into `products.image_url`. Product images now live in the `product-images` Supabase Storage bucket, and `img-src` is scoped to `'self' data: blob: <supabase-host>` in `src/proxy.ts`. An HTML-injection attacker can no longer beacon visitors' IPs via `<img src="https://evil.example/...">`.
...
*(The "if reopening this audit" checklist and the post-audit accepted-risk addendum that
used to close this report were living content, and moved to `docs/architecture/security.md`
— see "Continuous verification", "Next audit coverage", and the accepted-risks entry on
the password-reset `token_hash` query-string channel.)*
