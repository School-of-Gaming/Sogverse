...
- **Reports are ~500–1500 chars with light formatting** (title, headers, paragraphs,
  ...
  term of reports never becomes a wall of text. **The report editor is a real
  WYSIWYG from the mock onward** — gedus see formatting as they type, never raw
  markdown syntax (browsers offer no native rich textarea, so this needs a lib:
  Tiptap StarterKit, headless and styled by our tokens, basics-only toolbar —
  bold, italic, heading, lists — serializing to/from the canonical markdown).
  The gedu-note field stays a plain textarea.
...
- **Dashboard groups cards under the type nouns** — "Clubs", "Camps", "Events" as
  ...
  too generic; the nouns are the gedu's vocabulary). Scenarios cover both
  compositions: `default` carries all three types (including an event card) and a
  `clubs-only` scenario shows the single-type dashboard.
...
- **Minecraft username becomes a reusable fixed-geometry field**: the existing skin
  system renders the **full body** — the row uses a fixed **bust crop of that same
  asset** (CSS crop, no new system) to stay compact, with the full-body render kept
  as the component's large variant for roomier surfaces. Username + a fixed
  async-status slot (checking → valid/invalid via the existing Mojang validation);
  identical dimensions in every state so validation and editing never reflow the
  row.
