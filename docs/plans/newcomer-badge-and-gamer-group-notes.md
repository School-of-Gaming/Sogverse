...
session starts: a **newcomer badge** that fades across a member's first month in a group,
...
- A member who joins a group reads as **new to that group for 30 days**, and the badge
  **fades** across the window (opacity 1.0 → 0.3, then gone). The fade is the feature: a
  Gedu glancing at a roster reads not just "new" but "how new". The arithmetic is already
  written and unit-tested — `newcomerPresence()` in `src/components/member-flair/`.
...
### One decision is still open — do not decide it here

**Which note-indicator treatment ships.** Three candidates are on the table:

1. an avatar-corner dot (`GamerNoteDot` already exists as this candidate),
2. a trailing row icon,
3. an inline preview line of the note's first words.

This is **pending preview review** and is the owner's call, made by looking at the scenes.
Everything else in this plan is decided. Build the wiring so the indicator is a single
component swap; do not ship a preference by picking one quietly.
