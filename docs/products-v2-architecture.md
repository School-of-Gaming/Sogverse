...
### Gedu session-details page — unassigned-gamers tray

The Gedu session-details page (`get_gedu_assigned_product` RPC, behind `/gedu/{clubs,camps,events}/[id]`) returns `groups[]` only. The Gedu can see every group in the product they teach plus the full roster of their own group, but new signups that haven't been placed in a group yet (`participations_v2.group_id IS NULL`, `status = 'active'`) are invisible. Add a read-only "Awaiting assignment" section that lists those rows so a Gedu knows who's coming. They can't move gamers themselves — that's still admin-only via `commit_group_changes_v2`. Extend the RPC's return JSONB with an `unassigned[]` array (same shape as the admin RPC's existing field) and a section above the group cards. Drives the "I want to know who just signed up" nudge without granting write access.

### Gedu session-details page — error vs. "not assigned" are conflated

`SessionDetailsPage` branches `!data ? <NotAssignedState/>`. The service (`getAssignedProductDetail`) maps only the RPC's `42501` to `null`; every other error (transient 500, network blip) throws, so React Query leaves `data` undefined and the page renders "this isn't your session" — a misleading message for a server error. Fix: check the query's `isError` separately and render a distinct error/retry state, reserving `NotAssignedState` for the genuine forbidden (`null`) case. Low priority — the happy path and the forbidden path are both correct; only the error path shows the wrong copy.
