...
- **Shipping a *subset* of `dev` to `main` (hotfix) is a different operation — this command does not do it.** To hotfix: branch off `origin/main`, cherry-pick the specific commits, PR. **Do not reset `dev`** afterward — it still holds the originals plus the commits you held back. This deliberately creates twins; the next full `pr-dev-to-main` run will detect them at Step 2 and take the divergent path. If you do this often, give it its own `/hotfix-to-main` command rather than overloading this one.
