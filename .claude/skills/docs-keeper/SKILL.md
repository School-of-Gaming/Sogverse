...
- **Surgical updates.** Don't rewrite still-accurate sections — and judge "accurate" against the *current* policy: a refined policy makes the rule that owned its territory incomplete, so editing that rule is the surgical move, not appending a sibling beside it.
- **Edit or remove before adding.** Before writing a new rule paragraph or section, name the existing rule a reader would consult for this question and fold the refinement into it; a rule the refinement supersedes is rewritten or deleted in the same change, never left standing next to its replacement. A genuinely new heading has to pass one test: you can say why no existing rule could hold it. Why: rule files are always-loaded context, so every paragraph costs every session — and two rules sharing one territory can each be followed while their joint intent is violated, because a reader obeys the one they found.
