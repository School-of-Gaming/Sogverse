...
description: Create, re-point or verify the Stripe products webhook endpoint for a feature-branch preview, staging or production, including its signing secret in Vercel — and test checkout locally with stripe listen and the test cards.
---

# The Stripe products webhook across environments
...
The signing secret is `STRIPE_PRODUCTS_WEBHOOK_SECRET` — not `STRIPE_WEBHOOK_SECRET`, a dead name (`docs/architecture/products.md` §Billing has the env-var facts). Adding it to Vercel follows the `vercel-env-vars` skill: piped through the CLI, `--sensitive` on preview and production.
...
**This list is closed — say no to the reflex to extend it.** One-off checkouts create invoices so that a refund can be issued as a credit note, but nothing in Sogverse reads an invoice or credit-note lifecycle, so no invoice-creation or `credit_note.*` event belongs on any of the endpoints below. The route answers an unhandled event 200, so a speculative subscription only produces noise — which is why `charge.refunded` was removed when the write-only refunds ledger was dropped. `invoice.paid` stays for subscriptions; its handler returns early on an invoice with no subscription, so a one-off invoice writes no second payment row.
...
Local CLI auth (device key provisioning, live-mode grants) is per-machine setup, not repo procedure — it isn't covered here.

## 1. Preview (feature branch on PR open)

Repeat this if a future feature branch needs its own webhook.
...
## 2. Staging (when the branch merges to `dev`)

The `dev` branch's preview URL is stable but a different host than the feature-branch preview, so the existing webhook needs to be re-pointed (or a fresh one created and the old one deleted). Re-pointing is simpler — the signing secret stays the same and Vercel needs no change.
...
stripe webhook_endpoints create --live --confirm \
...
**Verification:** after cut-over, send one real `$0.50`-class purchase through to confirm the live webhook is wired before announcing.

## Local development

`STRIPE_PRODUCTS_WEBHOOK_SECRET` is not required in `.env.local` for normal local dev. `stripe listen --forward-to localhost:3000/api/webhooks/stripe/products` prints a fresh `whsec_...` per session — paste that into the running process's env or your shell, not into committed-template files. `.env.local.example` documents the variable name only. Do **not** check in any real `whsec_...` value.

Test-mode cards for a local or staging checkout — any future expiry, any 3-digit CVC:
...
The full list is Stripe's testing docs (https://docs.stripe.com/testing).
