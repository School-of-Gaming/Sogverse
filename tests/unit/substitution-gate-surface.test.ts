 ...
 * for the substitution branch as well, or says in writing why it does not.**
 *
 * A gedu substituting a session is not assigned to its group — that is the whole
 ...
 * thing a machine cannot derive: does a missing substitution branch here mean a bug,
 * or the design?
 *
...
/**
 * What counts as reaching for the substitution branch: the requests table itself, or
 ...
 * each with the reason a missing substitution branch is the design rather than a gap.
 *
...
const ASSIGNMENT_ONLY = new Map<string, string>([
  [
    ...
    "A filter over the admin product list: it asks which gedus are ASSIGNED to each product so an admin can narrow the list by one of them. It gates nothing, and a substitution is not a standing property of a product — a sub who substituted one afternoon is not one of a club's educators, and listing them as one would make the filter answer a different question from the one it asks.",
  ],
  [
  ...
  [
    ...
    "Names the table in prose, to say which rows the assignment arm of `get_my_assigned_products` comes from. The substitution arm of that same read is carried on the row's own `kind` discriminator, which this file maps; there is no gate here to widen.",
  ],
]);
...
describe("the substitution branch on every gate named in TypeScript", () => {
  ...
  it("classifies every file: the substitution branch, or a written reason", () => {
    ...
    expect(
      unclassified,
      `These files name ${ASSIGNMENTS} without reaching for the substitution branch and without an annotation.\n` +
        `Either widen them (a substituting gedu is not assigned to the group they substitute on), or add an entry to\n` +
        `ASSIGNMENT_ONLY in this file saying why a missing substitution branch is the design:\n  ` +
        unclassified.join("\n  "),
    ).toEqual([]);
  });
  ...
  it("keeps the annotated list honest — no entry outlives its file", () => {
    ...
    expect(
      ...
      "These ASSIGNMENT_ONLY entries no longer describe anything — the file has " +
        "stopped naming the table, or it now carries the substitution branch. Delete them:\n  " +
        stale.join("\n  "),
    ).toEqual([]);
  });
