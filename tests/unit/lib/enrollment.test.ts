...
describe("getNextSessionStart", () => {
  ...
  it("cron scenario: hourly check finds session within charge window", () => {
    ...
    expect(isWithinChargeWindow(nextSession, { windowHours: 24, now: cronNow })).toBe(true);
  });
  ...
  it("cron scenario: hourly check finds session outside charge window", () => {
    ...
    expect(nextSession.toISOString()).toBe("2026-02-27T13:00:00.000Z");
    expect(isWithinChargeWindow(nextSession, { windowHours: 24, now: cronNow })).toBe(false);
  });
});
...
describe("isWithinChargeWindow", () => {
  it("returns false when session is far in the future", () => {
    ...
    expect(isWithinChargeWindow(nextSession, { windowHours: 24, now })).toBe(false);
  });
  ...
  it("returns true when exactly at the window boundary", () => {
    ...
    expect(isWithinChargeWindow(nextSession, { windowHours: 24, now })).toBe(true);
  });
  ...
  it("respects custom window hours", () => {
    ...
    expect(isWithinChargeWindow(nextSession, { windowHours: 6, now })).toBe(true);
    expect(isWithinChargeWindow(nextSession, { windowHours: 2, now })).toBe(false);
  });
});
...
describe("getRefundEligibility", () => {
  ...
  it("returns eligible with full refund when charged for upcoming session and outside window", () => {
    ...
    const result = getRefundEligibility({
      product, lastChargeSessionDate: "2026-02-27", windowHours: 24, now,
    });
  ...
  it("denies refund after session already attended (not_yet_charged)", () => {
    ...
    const result = getRefundEligibility({
      product, lastChargeSessionDate: "2026-02-27", windowHours: 24, now: saturday,
    });
  ...
  it("handles Postgres TIME format (HH:MM:SS) for start_time", () => {
    ...
    const now = new Date("2026-02-25T12:00:00Z");
    const result = getRefundEligibility({
      product: productWithSeconds, lastChargeSessionDate: "2026-02-27", windowHours: 24, now,
    });
