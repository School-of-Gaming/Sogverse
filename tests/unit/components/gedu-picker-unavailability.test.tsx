...
/** The row for one person, found by the name it renders. */
function rowFor(firstName: string): HTMLButtonElement {
  const row = screen.getByText(firstName).closest("button");
  if (row === null) throw new Error(`No row rendered for ${firstName}`);
  return row;
}
...
describe("the gedu picker's caller-supplied refusals", () => {
  it("disables each named candidate and badges the caller's own reason", () => {
    ...
    const cases = [
      ["Eeli", "admin.products.geduPicker.alreadyAssigned"],
      ["Saana", "admin.products.geduPicker.alreadyExpected"],
      ["Milo", "admin.products.geduPicker.absentGedu"],
    ] as const;
  ...
  it("refuses an uncertified candidate the caller said nothing about", () => {
    ...
    // And nobody else: with no map at all, the four certified rows stand.
    expect(rowFor("Eeli").disabled).toBe(false);
  });
