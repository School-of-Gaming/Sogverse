...
/**
 * Tests for commit_group_changes_v2 + get_product_groups_v2_with_details.
...
describe("commit_group_changes_v2", () => {
  ...
  describe("adding groups", () => {
    ...
    it("creates a group and returns the temp id mapping", async () => {
      const { data, error } = await adminAuth.rpc("commit_group_changes_v2", {
    ...
    it("rejects a blank name", async () => {
      const { error } = await adminAuth.rpc("commit_group_changes_v2", {
        p_product_id: PRODUCT_BASIC,
  ...
  describe("deleting groups", () => {
    ...
    it("cascades to gedu assignments", async () => {
      ...
      await adminAuth.rpc("commit_group_changes_v2", {
        p_product_id: PRODUCT_DELETE,
    ...
    it("ignores groups that belong to a different product", async () => {
      ...
      // Cleanup.
      await admin.from("product_groups_v2").delete().eq("id", otherId);
    });
  });
  ...
  describe("Gedu assignment swap across groups", () => {
    ...
    it("moving a Gedu from group A to group B in one call clears the unique conflict", async () => {
      const setup = await adminAuth.rpc("commit_group_changes_v2", {
  ...
  describe("participation moves", () => {
    ...
    it("rolls back the entire batch if any single statement fails", async () => {
      ...
      await admin.from("gedu_group_assignments_v2").insert({
        group_id: groupA,
        gedu_id: GEDU_A,
        product_id: PRODUCT_MOVES,
      });
  ...
  describe("validation", () => {
    it("returns Forbidden when called by a non-admin", async () => {
      ...
      const { error } = await customerAuth.rpc("commit_group_changes_v2", {
    ...
    it("denorm trigger rejects a manual insert with mismatched product_id", async () => {
      ...
      const { error } = await admin.from("gedu_group_assignments_v2").insert({
        group_id: groupId,
        gedu_id: GEDU_A,
...
describe("get_product_groups_v2_with_details", () => {
  let admin: SupabaseClient<Database>;
  let adminAuth: SupabaseClient<Database>;
  ...
  beforeAll(async () => {
    ...
    await deleteV2TestProducts(admin, [PRODUCT_DETAILS]);
    await createV2TestProduct(admin, { id: PRODUCT_DETAILS, seatCount: 50 });
  });
