...
import { ensureTestTopicV2, deleteV2TestProducts } from "./v2-helpers";

/**
 * DB-level coverage for update_product_v2(). Cousin of the create RPC,
...
describe("update_product_v2", () => {
  let admin: SupabaseClient<Database>;
  let topicId: string;

  beforeAll(async () => {
    admin = createAdminTestClient();
    topicId = await ensureTestTopicV2(admin);
  });

  afterAll(async () => {
    await deleteV2TestProducts(admin, [PRODUCT_ID]);
  });

  // Recreate a fresh product before each path so we're testing update,
  // not the residue of a previous test. Bypassing create_product_v2() and
  // inserting directly so we control exactly what's there to wipe.
  async function freshProduct(): Promise<void> {
    await deleteV2TestProducts(admin, [PRODUCT_ID]);
    await admin.from("products_v2").insert({
      id: PRODUCT_ID,
    ...
    // something to delete.
    await admin.from("product_translations_v2").insert([
  ...
  it("admin can update parent fields and wipe-and-replace children", async () => {
    ...
    const { data, error } = await admin.rpc("update_product_v2", {
      p_id: PRODUCT_ID,
      p_billing_mode: "paid",
      p_translations: [
  ...
  it("preserves stored status across an update", async () => {
    ...
    const { error } = await admin.rpc("update_product_v2", {
      ...
      p_timezone: "Europe/Helsinki",
      p_registration_opens_at: new Date().toISOString(),
      // Required by chk_products_v2_seat_count_null_requires_free when
  ...
  it("non-admin (customer) is rejected with 42501", async () => {
    ...
    const { error } = await customer.rpc("update_product_v2", {
      p_id: PRODUCT_ID,
      p_billing_mode: "paid",
