...
/**
 * Build the `from(...)` router used by the v2 branch. The branch queries
 * three tables in order — `product_groups_v2`, `participations_v2` (gamer
 * only), `gedu_group_assignments_v2` (gedu only) — so the mock dispatches
 * by table name and returns the right thenable shape for each.
 */
function mockV2Tables(opts: {
  group: {
    timezone?: string;
    is_remote?: boolean;
    slots?: Array<{ weekday: number; start_time: string; duration_minutes: number }>;
  } | null;
  participation?: { id: string; signed_up_at: string } | null;
...
describe("POST /api/voice/token — v2 branch ({ groupId })", () => {
  ...
  it("returns 400 when neither roomId nor groupId is provided", async () => {
  ...
  it("returns 404 when the product is in-person (is_remote = false)", async () => {
    authAs("gedu-id", { role: "gedu", first_name: "Edu" });
    mockV2Tables({ group: { is_remote: false }, geduAssignment: { group_id: GROUP_ID } });
    const res = await POST(tokenRequest({ groupId: GROUP_ID }));
    expect(res.status).toBe(404);
  });
  ...
  describe("membership gate", () => {
    it("rejects a gamer with no active participation", async () => {
      ...
      mockV2Tables({ group: {}, participation: null });
      const res = await POST(tokenRequest({ groupId: GROUP_ID }));
      const data = await res.json();
      expect(res.status).toBe(403);
    ...
    it("rejects a gedu not assigned to the product", async () => {
      authAs("gedu-id", { role: "gedu", first_name: "Edu" });
      mockV2Tables({ group: {}, geduAssignment: null });
      const res = await POST(tokenRequest({ groupId: GROUP_ID }));
      const data = await res.json();
      expect(res.status).toBe(403);
  ...
  describe("session window gate", () => {
    it("rejects when no slot has an open window", async () => {
      authAs("gedu-id", { role: "gedu", first_name: "Edu" });
      mockV2Tables({ group: {}, geduAssignment: { group_id: GROUP_ID } });
      mockComputeSessionWindow.mockReturnValue({
        isOpen: false,
  ...
  describe("mid-session enrollment gate (gamers only)", () => {
    it("rejects a gamer whose signed_up_at is after the current session start", async () => {
      authAs("gamer-id", { role: "gamer", first_name: "Kid" });
      mockV2Tables({
        group: {},
        participation: {
          id: "participation-1",
          signed_up_at: new Date(Date.now() - 30_000).toISOString(),
        },
      });
    ...
    it("allows a gamer who enrolled before the current session started", async () => {
      authAs("gamer-id", { role: "gamer", first_name: "Kid" });
      mockV2Tables({
        group: {},
        participation: {
          id: "participation-1",
          signed_up_at: new Date(Date.now() - 7 * 86400_000).toISOString(),
        },
      });
      mockIsEnrolledForSession.mockReturnValue(true);
      const res = await POST(tokenRequest({ groupId: GROUP_ID }));
  ...
  describe("response shape", () => {
    it("returns {token, roomUrl, role} — same shape as the v1 branch", async () => {
      authAs("admin-id", { role: "admin", first_name: "Boss" });
      mockV2Tables({ group: {} });
      const res = await POST(tokenRequest({ groupId: GROUP_ID }));
      const data = await res.json();
