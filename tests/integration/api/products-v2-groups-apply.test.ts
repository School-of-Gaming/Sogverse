...
import { POST } from "@/app/api/admin/products-v2/[id]/groups/apply/route";
import type { BatchGroupChangesV2 } from "@/services/groups-v2";

// The apply route is a thin wrapper around commit_group_changes_v2 — it
...
const emptyBatch: BatchGroupChangesV2 = {
  addedGroups: [],
  renamedGroups: [],
  deletedGroupIds: [],
...
function createRequest(body: unknown): Request {
  return new Request("http://localhost/api/admin/products-v2/x/groups/apply", {
    method: "POST",
...
describe("POST /api/admin/products-v2/[id]/groups/apply", () => {
  ...
  it("forwards the batch to commit_group_changes_v2 in the right shape", async () => {
    mockAuthenticatedAdmin();
    mockRpc.mockResolvedValue({ data: { tempMap: {} }, error: null });

    const batch: BatchGroupChangesV2 = {
    ...
    expect(mockRpc).toHaveBeenCalledWith("commit_group_changes_v2", {
