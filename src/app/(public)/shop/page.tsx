 ...
 * events in one fetch), the seat counts keyed on those ids, and the
 * spoken-language reference set (the filter strip's Language row). The results
 * seed React Query via `initialData` (ShopBrowse → ProductBrowsePage →
 * ProductBrowseFilters) so the grid and filters paint fully on the first frame
 * — no spinner, no row popping in late (CLAUDE.md layout-shift rule). The
 * client hooks still refetch on mount; this prefetch only affects the initial
 * render.
 *
