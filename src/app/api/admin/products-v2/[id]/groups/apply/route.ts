...
/**
 * POST /api/admin/products-v2/[id]/groups/apply
 *
 * Applies a batch of staged group changes via the commit_group_changes_v2 RPC.
...
export async function POST(
  ...
  let batch: BatchGroupChangesV2;
  try {
    batch = (await request.json()) as BatchGroupChangesV2;
  } catch {
  ...
  const { data, error } = await supabase.rpc("commit_group_changes_v2", {
