 ...
 * hide is only what a member's own browser already cached, the same "they
 * already received it" exposure the hidden-body wire rule records for text.
 *
 ...
 *
 * **The bytes are cacheable per user, forever**: the object is immutable
 * (`upsert: false`, named by a primary key that cannot recur, never
 * rewritten), so `private, immutable` at a STABLE URL lets the browser make
 * every re-render, remount and reload free — the property the rotating signed
 * URLs could never have. `private` keeps shared caches out of it; the CDN in
 * front of the app never stores a child's picture.
 */
