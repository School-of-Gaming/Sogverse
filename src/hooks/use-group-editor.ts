...
export interface EffectiveGroup {
  ...
  gedus: Array<GroupGeduDetail & { isPending: boolean; isPendingRemove: boolean }>;
  /** isMoved=true when this participation was moved here in the current edit session. */
  participations: Array<GroupParticipationDetail & { isMoved: boolean }>;
}

export interface EffectiveSnapshot {
  groups: EffectiveGroup[];
  unassigned: Array<GroupParticipationDetail & { isMoved: boolean }>;
}

/** @visibleForTesting */
export function computeEffectiveSnapshot(
  server: ProductGroupsSnapshot,
  state: GroupEditorState,
): EffectiveSnapshot {
  ...
  const serverParticipationById = new Map<
    string,
    { detail: GroupParticipationDetail; serverGroupId: string | null }
  >();
  ...
  // same product).
  const allServerGedus = new Map<string, GroupGeduDetail>();
  for (const g of server.groups) {
  ...
  const buildExistingGroup = (
    ...
    const gedus: EffectiveGroup["gedus"] = sg.gedus.map((ge) => ({
      ...ge,
      isPending: false,
    ...
    const participations: EffectiveGroup["participations"] = [];
  ...
  const groups: EffectiveGroup[] = server.groups.map(buildExistingGroup);

  for (let i = 0; i < state.addedGroups.length; i++) {
    const ag = state.addedGroups[i];
    const gedus: EffectiveGroup["gedus"] = ag.gedus.map((ge) => {
...
export function useGroupEditor(server: ProductGroupsSnapshot | undefined) {
  const [state, dispatch] = useReducer(reducer, initialState);

  const empty = useMemo<ProductGroupsSnapshot>(
