...
export interface EnrollmentFixtureSpec {
  ...
  isRemote: boolean;
  /**
   * The product's topic, which decides whether the card has a "Before the first
   * session" guide to offer and which one.
   *
   * **Optional, and it defaults to a topic that carries no guide**, so a
   * fixture that is not about the prep affordance renders none and reads
   * exactly as it did before there was one. A fixture that *is* about it names
   * a topic with a guide — `roblox_studio` is the long one, three steps with
   * per-platform notes and a checklist, which is the one worth judging a
   * scrolling dialog against.
   */
  topic?: ProductTopic;
  slots: FixtureSlot[];
...
export function buildEnrollmentFixture(
  ...
  return {
    ...
    // A label-only topic by default: it carries no prep guide, so an untouched
    // fixture draws no prep affordance and reads exactly as it did before the
    // affordance existed.
