...
**Rule: a room is not a roster — the note affordance appears only on the rows of people who hold a seat in the group.** Who is in a voice call is whoever holds a token for it: the group's members, plus the Gedu running the session, plus anyone covering, plus any admin who dropped in. A Gedu note is keyed to `(group, participant)` and the write RPC's target check asks whether the person currently sits in that group, so every one of those staff is an illegal target — a Gedu cannot take a note about themselves. The overlay therefore carries the group's seat-holder ids alongside its notes and stamps, and the list gates the button on that set. Gating on the row's *role* instead would be wrong in a real case: a Gedu can hold a seat in some other group, and the question is membership of **this** one. Without the gate the button renders, the save goes out, and the database refuses it — the right answer arriving in the worst possible place.

**Rule: the staff-only overlay on a participant row rides its own channel, never the Daily token.** A newcomer badge and the existence of a Gedu note are staff working memory about a child; `user_name` is broadcast to every peer in the room, children included, and lives as long as the token does. So the overlay reaches the list through `VoiceMemberFlairProvider` — a context supplied by whatever page mounted the room, filled from a staff-scoped read that a family's client never makes and RLS would refuse it anyway. Absent by default: with no provider the rows render exactly as they did before the overlay existed, which is what a child's room gets. Resolve it once for the list and hand each row its own answer, exactly as the figure lookup below does — it is a fact about the *viewer*, not about any one row.
