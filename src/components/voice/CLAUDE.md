...
A parent is a participant with no moderator capabilities — guest-equivalent, exactly like a gamer. Every *moderator* gate is a positive `admin`/`gedu` check (client-side) or `is_voice_group_moderator` (RLS, which is admin-or-assigned-gedu), so the column falls out with no UI code. One non-moderator branch does allow-list roles: the Minecraft badge renders for `gamer`/`gedu` only, so a parent shows none — moot today, since parents cannot link game accounts (deliberately deferred; see the products-for-parents follow-ups), and the badge question rides with that deferral rather than being decided here. Joining their child's room is a different thing entirely and still goes through the switch-to-gamer flow.
