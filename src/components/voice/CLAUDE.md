...
**Rule: Daily.co owns room existence and live participant/presence state — there is no `voice_rooms` table, no `daily_room_name` column, no presence table, no scheduler that pre-creates rooms.** New groups need no provisioning; deleted groups need no room cleanup (rooms self-expire). Live presence and the room's own mic lock are deliberately ephemeral — nothing records who was in the call, or who was silenced in it. **Chat is the exception, and it is not Daily's at all**: it persists in its own tables over its own transport (see In-call chat below), so a late joiner reads what was said before they arrived and a gedu can remove a message after the fact.
...
**Chat is not a counter-example to that, because it touches neither table.** It is its own subsystem — its own tables, its own RLS, its own realtime channel, keyed by the same `(group, session window)` pair the room name is derived from rather than by anything Daily knows. So the line this principle draws holds: what a room *is* and who is in it stay Daily's, and a second answer to either of those questions does not belong in the database.
...
**Rule: Get-or-create the Daily room on demand, never pre-create.** First joiner creates it, everyone after reuses it: `GET /rooms/{name}`; on 404 `POST /rooms`; on a duplicate-name race re-`GET` the winner. Daily returns the duplicate as `400 invalid-request-error` ("already exists") — not 409 — so detect it with `isDailyDuplicateRoomError`, never by branching on HTTP status. **Neither expected response (the probe miss, the duplicate) logs at error level** — creation leaves an info-level `[daily] created room=…` breadcrumb instead, the one per-room trace of lazy creation (load-bearing observability, not leftover debug output) — so a Daily error line in prod logs is always genuinely wrong, never this flow working as intended.
...
3. **Membership** — seat-holder (gamer **or** customer): active `participations` row for `(group_id, participant_id)`. One query serves both — `participant_id` is whoever occupies the seat, so a parent's own seat on a for-parents product satisfies it and a parent has no path to their *child's* seat (that row carries the child's id). Gedu: a `gedu_group_assignments` row on **product_id** (cross-group voice mobility), **or** a live `session_substitution_requests` row seating them as this group's sub for **today in the product's timezone**. The substitution arm is date-scoped where the assignment arm is not — a sub joins the room on the day they substitute and on no other day of the group — and it adds to the assignment arm rather than narrowing it. Admin: bypass.
...
|---|---|---|---|---|
| Join (in window) | any group | assigned product, or a group they substitute on today | active participation | active participation on their **own** seat |
...
A parent is a participant with no moderator capabilities — guest-equivalent, exactly like a gamer. Every *moderator* gate is a positive `admin`/`gedu` check (client-side) or `is_voice_group_moderator` (RLS, which is admin-or-assigned-gedu), so the column falls out with almost no UI code. The one piece of UI a parent does get is the participant row's identity slot, which follows the adult-variant grammar the rosters established: a child-shaped fact for children (the game identity, rendered for `gamer`/`gedu` only), the shared "Parent" role badge for a customer — the same slot answers "who is this" either way, so a gedu glancing at the room can tell the adult from the children. The badge has **no viewer gate** — every peer in the room, children included, sees it. That is the one new child-facing exposure parent seats create, and it is deliberate: an adult being visibly present in a room with children is a safeguarding *positive*, not a leak, and the token carries only what a child's already does (a first name, no surname, no email — plus the room's own platform slot, which a customer's row never draws). Parents show no game identity because they cannot link game accounts — deliberately out of scope when parent seats were built. Joining their child's room is a different thing entirely and still goes through the switch-to-gamer flow.
...
**The icon/color sets are app-owned, not DB enums.** `voice_zones.icon` / `.color` are plain `text` columns; the `VOICE_ZONE_ICON_KEYS` / `VOICE_ZONE_COLOR_KEYS` tuples in `voice-zones.ts` are the valid sets and the picker order (they drive the derived `VoiceZoneIcon` / `VoiceZoneColor` types too). So adding/removing/renaming one is a pure code change — no migration. The renderer resolves a stored key through `zoneIconFor` / `zoneColorFor`, which fall back to a default glyph/colour for any unknown or removed key, so an old row never breaks. (This deliberately departs from the "derive enums from generated `Constants`" convention in `src/CLAUDE.md` — the value set is presentational and mod-gated, not a security boundary.)

**The colours are not this app's to define.** They are SOG-UI's sixteen picks — the colours a person may choose for their own thing — and a zone colour key is one pick's id spelled as text (`"1"` … `"16"`), because the column is text. `voice-zones.ts` decides only how a pick is painted onto a zone; the hues, their order and the reasoning behind them are the library's. A zone colour therefore says "a moderator picked this" and never a status, a kind or anything the app assigned by meaning — so a swatch is labelled by its number (`voice.zoneColor`, one string per locale, with the id substituted) rather than by a hue word. Each *icon* key still needs its own label under `voice.zoneIcon.*` in every `messages/` locale.
...
## Local mic/camera state: intent, not echo

**Rule: the local user's own mic and camera on/off is *intent* — synchronous truth owned by the provider — and is never derived from Daily's local track state.** Exactly four things write it: the user's own toggle; the seed taken once at join — mic and camera each from the same value the corresponding meeting-token property was minted from, so intent and token agree by construction rather than by reading one back from the other; an owner-verified moderator app message; and the reset on leave. Nothing else may set it, and no code path may re-derive it from the call object.

**Daily's local track state is *health*, and it feeds only the media-error surface** — the badge on the mic-settings chevron and the notice inside the popover. That surface is the **microphone's** health, so it is derived from the local *audio* track alone: a `blocked` track maps through its own blocked-reason object onto the acquisition categories (no permission, no device, device held elsewhere), `interrupted` is its own category, and `playable` or `off` mean nothing is wrong — the mic is working, or its owner turned it off, and a mic that could not be acquired reads as blocked rather than off, so off hides nothing. A blocked track naming no reason keeps whatever was already reported: Daily's `camera-error` event stays the source for the failures the blocked object cannot express (an insecure context, unsatisfiable constraints), and a vaguer answer must not overwrite one of those. One pure function folds all of that, and it is the whole of what may read a local track state. Health never touches on/off. The camera has no health surface of its own — a camera toggle that throws writes its error into the same channel, where it lasts until the next mic-health tick — which is an accepted limitation of one channel serving two devices, not a property to build on.

The split is forced by three facts about the SDK, and conflating the two produced a mic that appeared to re-mute itself. `interrupted` means the track exists and the user has *not* turned it off — the browser has merely reported the underlying device as not delivering for a few seconds, an OS/Bluetooth condition — and daily-js itself counts that as **on**. Enabling a local track is asynchronous: the call is a message to the call machine and reports nothing back, so any read-back races the click that caused it. And the participant-update handler runs on every Daily event, including the active-speaker change that fires constantly in a room where people talk. So the old read-back turned a stalled device into a UI that said "muted", made the user's next click a no-op that flipped the button back to "on", and then flipped it to "muted" again on the very next event — the button lying and the click doing the wrong thing, both from the same cause. A device that stalls is a thing to *report*, never a decision to mute somebody.

This is the same rule as the local user's own position: one synchronous source of truth for what *you* did, with the echo demoted to observing everyone else. Anything new that wants to know whether your own mic is on reads the intent state; anything that wants to know whether it is working reads the health channel.
...
- **Mute** — one-shot `updateParticipant(sid, { setAudio: false })`; the target can re-enable. The SFU call changes the *track*; the accompanying owner-verified app message is the only channel by which a remote mute reaches the target's intent state (never the track echo — see the intent-not-echo rule above), and the target acts on that message by stopping its own track as well as writing its own state, so the two agree by construction. The moderator's SFU call still lands, and the two are idempotent together.
...
- **Chat lock** — the odd one out, and the row's menu is only where it is *offered*: it is a database row on the chat channel rather than anything Daily knows, it survives the session, and whether it appears at all is decided by the chat surface (see In-call chat below). It reads as the third member of the menu's lock group and sits last in it, because it is the only one of the three that can be absent. The menu grows *upward* from its trigger, so nothing may be added to it while it is open — a fourth item would push the three on screen up under a hand reaching for one; the chat panel above the rail has resolved long before any menu is opened, which is what makes that safe rather than the ordering.
...
**Rule: the room and its chat derive the same session window from the same schedule, in two implementations — and where two slots' windows overlap they can disagree about which one.** The token route walks the product's slots in whatever order PostgREST returned them and takes the first whose window is open; the chat RPC takes the earliest-*opening* open window, deterministically, because two joiners racing on mount must not materialize two channels for one room. On any ordinary schedule exactly one window is open and both answers are the same one. When two overlap, the room name can be derived from one slot's window while the channel is keyed to the other's — one chat log for what the schedule describes as two overlapping sessions, which is a scheduling shape nothing else in the product handles either. Neither side loses data and no boundary moves: the channel is still keyed to a real window of this group, and the family read bound is still that window. A migration comment claims the failure mode is only ever the chat-unavailable state; that holds for a *boundary instant* disagreement, not for this.

**Three callers, three answers, and the slot is why one layout serves all three.** The scheduled session page passes the live container (`GroupSessionChat.tsx`), which holds the channel, the history read, the one realtime subscription and every write; the voice preview scenes pass the same components over fixtures; an instant room passes nothing and no card is drawn (see `./instant/CLAUDE.md`).

**Rule: the moderator's chat lock is offered on the participant row too, and it arrives as a prop rather than as a second derivation.** A lock is a judgement about a person, so a moderator can lift one beside somebody's *name*; the alternative is hunting the log for a message that child happened to write. What the room is handed is a function of a user id returning the conclusion (which way the switch points, and what to call), so nothing in this directory tests a role, asks who is on the chat roster, or knows which way a lock is currently set. The single answer lives in chat's own capability module and the message menu goes through the same function; the rail renders whichever of two labels it is told, in chat's own words.

**The state is published *upward* out of the chat container, and that is deliberate rather than lazy.** The rail and the chat panel are siblings inside the room, and the lock rows arrive on the chat history read that `GroupSessionChat` holds — a second reader of that query elsewhere in the tree would fire the history fetch before the subscription's ack, which is exactly the ordering that container exists to keep. So the container publishes what it already knows to whoever mounted it, the page threads it into the room, and the value is memoised on the roster and the standing locks so ordinary chat traffic republishes nothing and the room around it does not re-render.
...
**Rule: which game identity a room shows is the *product's* decision, resolved once at token-mint, and every peer in a room carries the same one or none.** The token route maps the product's topic to a platform: a Minecraft topic reads the Minecraft account table, a Roblox one reads the Roblox table, and every other topic is about no single game account — no read at all, and a token with no game slots. Nothing downstream re-decides it: the client renders whatever platform the slot names, so a room cannot show one child's identity on a platform another child's row was drawn from.

**The platform slot is the gate, and it separates two different absences.** No platform means the room surfaces no identity (an instant room, or a topic about no game account) → the row hides the slot entirely. A platform with an empty username means "this room is about that platform and this person has no linked account" → the row renders "(Unknown)". Collapsing the two would put an empty Minecraft row on an Esports club.

**Rule: the token is the only identity channel, because the account tables' RLS forbids reading another user's row.** A per-participant client query is impossible, so each token carries only its owner's identity — read server-side, where a self-read is always allowed — and Daily broadcasts `user_name` to every peer.
...
**Rule: the parser tolerates every layout a live room can hold, because a token outlives a deploy.** A token is minted once at join and lives for the whole session window, so a deploy landing mid-window puts two formats in one room at the same time. The parser therefore reads the current six-slot form, the legacy five-slot Minecraft-only form (`…|username|uuid`, treated as platform `minecraft` — which is all it ever was), and the three-slot instant form. **A platform slot it does not recognise degrades to no game context rather than throwing**: these slots are cosmetic, and a peer minted by a newer deploy must drop its identity row, not blank itself out of everyone's roster. The role slot is the deliberate contrast — it drives moderator UI, our routes are its only writers, and a bad one throws.

**The accepted consequence: for the one session window that spans a deploy, a room can hold five-slot peers beside six-slot ones, and the five-slot ones are read as Minecraft.** So on a Roblox product — or one whose topic names no game account at all — a pre-deploy joiner can transiently show a Minecraft-shaped identity row, or an empty "(Unknown)" one, next to correctly-typed peers. That breaks the "every peer in a room carries the same platform or none" property for exactly as long as that token lives. It is cosmetic, it is bounded by one window, and it self-heals the moment the peer rejoins on a token the new deploy minted — which is why the parser tolerates the legacy layout rather than refusing it and blanking the peer out of the roster entirely.

The account key crosses as text and comes back in the shape its platform stores it in: a Mojang UUID is already a string, a Roblox id goes over as its decimal string and is parsed back to a number (a value that is not a clean integer reads as `null` — an unverified row, which draws the silhouette, where a `NaN` would reach a thumbnail endpoint expecting an id).

## UI

**Rule: The UI is a pure consumer of the `VoiceRoomProvider` context.** All state and actions live in the provider + hooks; components only render what they're given and call actions. This is why the voice room demos in `/admin/ui-components` with a hand-built mock context (see the `src/CLAUDE.md` note on that page) — and why the *visual* design is freely tweakable with zero risk to the logic.

- `VoiceRoom` — the in-session layout (title row, screen-share viewport, zone list, control bar, the chat slot, participant list). Its `titleAccessory` slot renders a caller-supplied control at the right of the title row and is empty unless passed — instant rooms put their room-link chip there; scheduled rooms have no shareable link and pass nothing, so the row stays a bare heading with no gap held open.
...
- `VoiceControls`, `ScreenShareDisplay`, `ParticipantList` / `ParticipantRow` (no volume slider), `GroupSessionChat` (+ its pure row-to-view-model turn beside it), `MicLevelIndicator`, `VoiceAvatar`, `JoinVoiceButton`.

**Rule: a room is not a roster — the per-gamer dialog's affordance appears only on the rows of people who hold a seat in the group.** Who is in a voice call is whoever holds a token for it: the group's members, plus the Gedu running the session, plus anyone substituting, plus any admin who dropped in. A Gedu note is keyed to `(group, participant)` and the write RPC's target check asks whether the person currently sits in that group, so every one of those staff is an illegal target — a Gedu cannot take a note about themselves. The overlay the room is handed therefore carries the group's seat-holder ids alongside its notes and stamps, and the list gates the button on that set. Those ids are a field of the *client-side* overlay value, derived by whatever builds it — the server document has one entry per active participation already, note or no note, so its keys are the set. Nothing needs to send a second array of ids over the wire. Gating on the row's *role* instead would be wrong in a real case: a Gedu can hold a seat in some other group, and the question is membership of **this** one. Without the gate the button renders, the save goes out, and the database refuses it — the right answer arriving in the worst possible place.

**Rule: the room draws the same per-gamer dialog the workspace does, and it is the whole dialog — private note and creations both.** A Gedu is never better placed to write down what a gamer just made than in the session they made it in, so the overlay carries a member's creations beside their note and the room's dialog edits both halves. What the room does **not** draw is the owed marker: whether a creation is *wanted* is a fact about the product's flag and the run's final session, and this document deliberately carries neither. That signal lives on the workspace, where the schedule is.

**Rule: the staff-supplied overlay on a participant row rides its own channel, never the Daily token.** A newcomer badge and the existence of a Gedu note are staff working memory about a child; `user_name` is broadcast to every peer in the room, children included, and lives as long as the token does. (Creations are the one entry on that channel that is not staff-only — the member's own family reads the same list through their own document — and they ride it anyway, for the reason above; what keeps the privacy line intact is that the read is staff-scoped and no child's client makes it.) So the overlay reaches the list through `VoiceMemberFlairProvider` — a context supplied by whatever page mounted the room, filled from a staff-scoped read that a family's client never makes and RLS would refuse it anyway. Absent by default: with no provider the rows render exactly as they did before the overlay existed, which is what a child's room gets. Resolve it once for the list and hand each row its own answer, exactly as the figure lookup below does — it is a fact about the *viewer*, not about any one row.

**Corollary: the room's flair clock is frozen at mount, and the derivation from the overlay document is a pure function beside the page.** The badge answers in whole days and a room lives an hour or two, so the shared ticking `useNow()` would buy a day boundary nobody is waiting for and charge a full flair rebuild (a fresh Set, four fresh maps) every 30 seconds — re-rendering the whole participant list on a page holding live media, a dnd-kit board and the speaking-glow analyser. One `useState(() => new Date())` at mount instead, which also keeps every row's badge agreeing with every other row's for the session. The document→context turn lives in `derive-voice-member-flair.ts` rather than inside the page: the clubs-only gate, the seat-holder set and "absence is how none is spelled" all fail *silently* when they fail, and none of them needs a Daily call or a React tree to test.

**Rule: on a participant row the name is the only thing that may truncate, and the game identity is the only thing that changes place.** A game account is never abbreviated — a clipped username is worse than useless, since checking it against the server is the whole reason it is on the row — so the identity slot is `shrink-0` and sized to its own content. It carried a fixed 160px column once; that both clipped the long names and left a dead gap after the short ones, so it is gone. Everything else on the row is `shrink-0` too, and the name has `min-w-0 truncate`: a long name gives way, nothing else does.

The width budget is why the identity moves. Inside the dashboard container at the 360px floor there are 294px of row content; the avatar, the status pair, the note button, the moderator menu and their gaps take 170, leaving 124 for the name, the badges *and* the identity on one line. It does not fit — and did not before the note button existed either. So **the row is one wrapping flex line whose children are ordered per breakpoint**: from `sm` up the identity sits directly after the name, and below `sm` it is `w-full` and last in order, which drops it onto a line of its own indented past the avatar while the name and its badges stay together on the first. That ordering is the reason every item is a direct child of the row rather than nested into halves — nesting cannot move one item between two positions. The row's slack is parked on the trailing control group with `ml-auto`, so those controls pin right on every row regardless of what the name, the badges or the identity did with the space before them. That group is always rendered and always holds something, which is what makes it a dependable sink; the badges beside the name have no wrapper of their own, because a wrapper would have had to decide whether it renders at all — the honest answer being the badge's own window arithmetic, not the row's — and an empty flex child still costs a gap on either side of itself.

Anything new competing for that line does the arithmetic again rather than eyeballing it, per the `src/CLAUDE.md` rule on fixed strips sharing 360px.

**Rule: a mark that can arrive after first paint goes at the end of the run it joins, and the trailing controls are one right-packed group for the same reason.** The row is painted from the Daily token the instant the room is; the staff overlay lands a round trip later. What it brings therefore has to be **additive by construction** rather than by luck, and two placements are what make that true:

- **The trailing controls — note button, status icons, moderator menu — are one group carrying the row's `ml-auto`, with the note button at that group's left edge.** Because the group is right-packed, its right edge is the row's right edge and its contents pack leftward from there, so a note button appearing later grows the group into the row's slack and the icons and the menu keep their positions to the pixel. The button sat *between* the icons and the menu once, and there it shoved the mic and camera left by its own width on every row, every time the overlay resolved — an already-painted element moving on data's own schedule, which the app-wide layout rule forbids outright.
- **The newcomer badge renders after the Parent badge and is the last item on the identity line.** It is the only thing on that line that can arrive late, so it goes at the end of the run, where its arrival is absorbed by the line's slack instead of displacing an adult's role badge. The gedu roster row orders the two the same way even though its flair arrives in the same payload as the roster, so there is one order across both surfaces and no surface has to remember why.

The corollary for anything added next: put it where the run ends, or resolve it before the row's first paint. Reserving space for it is the wrong third option — the row cannot know whether a member is inside the newcomer window or has a note, so a held-open slot is a hole on most rows to prevent a shift on few.

**Rule: a participant row never goes looking for its own figure — the list resolves them all and hands each row its answer.** The identity slot draws whatever picture it is given, and where a platform's renders have to be looked up (Roblox: a thumbnails API rate-limited per IP across the whole serverless fleet) a hook per row is N requests against a budget one busy session could drain. So `ParticipantList` collects the verified account ids of the current participants and passes each row a URL or an explicit `null`. A platform whose image host is addressable by username (Minecraft) needs no lookup and the row derives its own; that is the one case where the list passes nothing.

**Corollary: a room's membership moves, so the lookup accumulates rather than re-asking.** The batch a static roster uses is keyed by its whole id set, which is right for a snapshot and wrong here: every join and leave would be a different question, discarding the answer and re-asking about the whole room, so a ten-person room filling one person at a time costs fifty-five upstream calls instead of ten. This list uses the live variant, which keeps an ever-seen set beside the resolved map and asks **once per change, about only the people that change brought** — a join with no linked Roblox account, or anybody leaving, issues no request at all. An id is marked seen before its request goes out, so a second join asks only about the newcomer even while the first request is still in flight. A failed batch settles its ids as "no picture" and is never retried; the map is component state and is never persisted.

**Rule: the identity slot's geometry never moves, on either platform or while a picture is in flight.** It draws the `head` figure, which is square on Minecraft and Roblox alike, so a room's platform does not change the row's shape — and the figure box is at its final size before any render lands, so pictures arriving (or failing, which is never retried) shift nothing.

**Rule: Don't violate the `src/CLAUDE.md` layout rules** — no in-place shifts of already-rendered content; the `committing` pattern on dialog submits. (Entering a private zone is now an in-place `userData` change, not a reconnect, so there's no transition screen to keep chrome mounted through.)
...
Per-participant volume slider (wiring kept, UI dropped — see TODO.md); lock state surviving rejoins; participant presence persistence. All deferred — and in tension with the Daily-owns-presence rule, so weigh that before picking one up.
