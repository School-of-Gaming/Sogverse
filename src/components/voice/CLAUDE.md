...
**Rule: a participant row never goes looking for its own figure — the list resolves them all in one call and hands each row its answer.** The identity slot draws whatever picture it is given, and where a platform's renders have to be looked up (Roblox: a thumbnails API rate-limited per IP across the whole serverless fleet) a hook per row is N requests against a budget one busy session could drain. So `ParticipantList` collects the verified account ids of the current participants, asks once, and passes each row a URL or an explicit `null`. Ids change as people join and leave — that is the call's key, and a resolved id is not re-asked. A platform whose image host is addressable by username (Minecraft) needs no lookup and the row derives its own; that is the one case where the list passes nothing.
