...
A parent is a participant with no moderator capabilities — guest-equivalent, exactly like a gamer. Every *moderator* gate is a positive `admin`/`gedu` check (client-side) or `is_voice_group_moderator` (RLS, which is admin-or-assigned-gedu), so the column falls out with almost no UI code. The one piece of UI a parent does get is the participant row's identity slot, which follows the adult-variant grammar the rosters established: a child-shaped fact for children (the Minecraft identity, rendered for `gamer`/`gedu` only), the shared "Parent" role badge for a customer — the same slot answers "who is this" either way, so a gedu glancing at the room can tell the adult from the children. The badge has **no viewer gate** — every peer in the room, children included, sees it. That is the one new child-facing exposure parent seats create, and it is deliberate: an adult being visibly present in a room with children is a safeguarding *positive*, not a leak, and the token carries only what a child's already does (`<uuid>|customer|<FirstName>||` — a first name, no surname, no email). Parents show no game identity because they cannot link game accounts — deliberately out of scope when parent seats were built. Joining their child's room is a different thing entirely and still goes through the switch-to-gamer flow.
