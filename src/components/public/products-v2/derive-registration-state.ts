...
import type { ProductTypeV2, ProductV2 } from "@/types";
...
// post-`participations_v2` future: when participation counts go live,
...
export type RegistrationState =
  ...
  | {
      kind: "pending_thr";
      threshold: number;
      /** 0 today; real once participations_v2 ships. */
      count: number;
    }
  | {
      kind: "full_waitlist";
  ...
  | {
      ...
      /** `null` until participations_v2 ships. */
...
export interface DeriveRegistrationStateArgs {
  ...
  now: Date;
  /** Pass 0 until participations_v2 is wired up. */
  participationsCount: number;
}

const LATE_JOIN_LOCKED: Record<ProductTypeV2, boolean> = {
  consumer_club: false,
