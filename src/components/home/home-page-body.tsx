...
export function HomePageBody({
  // eslint-disable-next-line @typescript-eslint/no-unused-vars -- accepted now so the scene scenarios can pass it; the draft doses consume it in the design-pass commits that follow. The Yty elements section lives on /about since the About restructure, so home's first draft consumers are the hero and cards.
  palette = "current",
}: {
  /**
   * Which Yty palette the draft doses draw in. Defaults to the live one,
   * so the public route is byte-for-byte what it was; the home scene's
   * `brand-palette` scenario passes `"brand"`. Retires when the draft palette
   * promotes and the Yty tokens change value.
   */
  palette?: YtyPalette;
}) {
