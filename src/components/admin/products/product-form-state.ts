...
export function initialState(
  ...
  const locks = formLocksFor(
    config.productType,
    effectiveBillingMode(config, initialPaidMode),
  );
  ...
  const lockedConsumerStartDate =
    locks.consumerClubStartDateToday && config.productType === "consumer_club"
      ...
      : "";
  // Whether a fresh product of this type starts *capped*. A locked seat control
  // can only ever pin the safe default (uncapped, waitlist off). Where it is
  // unlocked the default is the type's own editorial choice, and only one type
  // has a capacity to begin with: a municipality club is contracted for a
  // specific number of places, so it starts capped + waitlisted and the blank
  // seatCount below forces the admin to type the contracted figure. A free
  // event may be capped but usually isn't, so it starts open like every other
  // type and a cap is opt-in.
  const startsCapped =
...
export function withPaidMode(
  ...
  const locks = formLocksFor(
    config.productType,
    effectiveBillingMode(config, paidMode),
  );
