...
import { ProductV2FormEdit } from "./product-v2-form-edit";
import { PRODUCT_TYPE_CONFIG } from "./product-v2-type-config";
import type { ProductTypeV2 } from "@/types";

interface EditProductV2PageProps {
  productType: ProductTypeV2;
  productId: string;
}

export function EditProductV2Page({
  productType,
  productId,
}: EditProductV2PageProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <h1 className="mt-1 text-2xl font-bold tracking-tight sm:text-3xl">
          {product
            ? t("editPage.title", {
                name:
                  resolveTranslation(product.product_translations_v2, uiLocale)
      ...
      {!isLoading && product && (
        <ProductV2FormEdit productType={productType} product={product} />
      )}
    </div>
  );
}
