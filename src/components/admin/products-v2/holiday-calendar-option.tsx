...
import type { HolidayCalendarWithDates } from "@/services/products-v2";
...
export function HolidayCalendarOption({
  ...
  const t = useTranslations("admin.productsV2.labels");
  const today = new Date().toISOString().slice(0, 10);
  const upcoming = calendar.calendar_holidays_v2.filter(
    (h) => h.date >= today
  );
