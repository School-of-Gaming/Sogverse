...
export const PREVIEW_SCENES = [
  ...
  },
  {
    ...
    description:
      "The public product page a parent lands on from the shop, with the registration signup panel in each of its states — and, at the end of each product type's run, the audience scenarios: the three shapes the picker takes once a product can be sold to parents. Those three are the only way to look at a for-parents product at all, since none exists yet and the admin switch that creates one lands last. The six *tagged* scenarios (€45/mo — open; Free — capped; Full with waitlist; awaiting threshold; the camp not yet started; and For families) render the **draft page body** instead of the live one, and it is a different page rather than a restyled masthead. A header band across the full width carries the back link, then the h1 with the type · topic eyebrow baseline-aligned at the far right. Beneath it the reading column leads with a 3:2 hero at the browse card's treatment, wearing the same two chips in the same corners — tag bottom-left, audience-or-age top-right — so a family meets the same pill on the page a card sent them to. The short description follows, and under it the tag explained: what SOG actually does about that tag, in a quiet block at the reading column's full width, with the tag's icon and word but deliberately no second pill (the hero above is already wearing it). The signup panel has moved into a sticky right rail, its picker flattened so a name, an age and “Already joined” fit one line at that width; from 2xl the overview card moves out again into a sticky left facts rail, label-over-value at rail width, leaving the reading column between them. The explanation copy is placeholder text written by an engineer and is being replaced wholesale by the product owner — read it for shape and length, not for wording. Every other scenario renders the live page untouched, which is what keeps the current design reviewable beside the draft.",
    chrome: "public",
