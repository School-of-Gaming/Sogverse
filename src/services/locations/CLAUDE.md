...
**Positive-selection semantics** (implemented in the coverage editor, not the DB):
- One tick is one independent claim and one row. Ticking a parent does **not** tick, pre-tick or half-tick its descendants, and unticking a descendant does not disturb any ancestor tick — a descendant showing as ticked would make the UI say something the saved rows do not.
- An empty selection is valid — the gedu is remote-only.
- A tick is identified by `(country_code, type, external_code)`, the same key the catalogs are built on, so a UI that only ever handled catalog entries resolves its ticks to row ids at save time. Resolution is a lookup, not an assertion: a code with no row must be **refused with the place named**, never silently dropped.
- Rows the catalog cannot show — sites (no official code) and anything in a country that ships no catalog — stay valid for matching and render as removable read-only chips.
...
- **`catalog-picker.tsx`** — the catalog browse/search panel, the dialog shell every catalog dialog shares, and the two dialogs built on it. The panel is handed a catalog and a **selection mode**: `single` confirms one leaf (how a place enters the database), `multi` puts a checkbox on every level for coverage ticking and additionally indexes the levels above the leaves so a search finds a département and not only communes spelled like it. The shell picks the country, loads that catalog behind a **dynamic `import()`** (code-split — France's is ~890 KB, so it must never be in the main bundle), and holds one fixed height across loading, failure and loaded. Search is instant and client-side over a pre-normalized index built once per catalog, diacritic-insensitive in both directions, capped at a rendered-result budget with the true match count still reported. An empty search box falls back to hierarchical drill-down, so browse and search share one panel with no mode switch.
...
- **Gedu coverage** (`src/components/gedu/`) — built on the catalog panel's multi mode, not the tree. The shared coverage field renders the current claims as a fixed-height box of chips and opens the catalog dialog to change them; the editor wraps it with a save, and the public register-gedu form embeds it directly and resolves its ticks at submit (`locations` is anon-readable, so an account need not exist yet). The tick semantics, the row→tick split and the resolution pairing are pure helpers next to the components, so they are unit-tested without mounting anything.
