 ...
 *
 * There are two reads under this key — the pool of open requests a gedu could
 * take, and the admin Substitutions page's whole document — and every
 * substitution write invalidates the root rather than either leaf: approving an
 * offer removes a line from the pool *and* moves a row from one half of the
 * admin page to the other, and a mutation that had to name each affected leaf
