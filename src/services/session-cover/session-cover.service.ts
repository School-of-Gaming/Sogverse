...
/**
 * Reads and writes for session covers.
 *
 * **Every method is an `.rpc()` call, and there is no API route anywhere in
 * this feature.** Both cover tables grant `authenticated` nothing, so the
 ...
 * server-side secret.
 *
 * **Refusals are left to throw.** A cover write is always somebody pressing a
 * button, and every refusal is news they have to be told: the session was
 * covered while the dialog was open, the offer went stale, the request is
 ...
 */
export class SessionCoverService {
  constructor(private supabase: AppSupabaseClient) {}

  /**
   * Every open request this gedu could take, ordered by date then product.
   *
   * The exclusion is the database's own *may cover* predicate rather than a
   ...
   */
  async getOpenRequests(): Promise<OpenCoverRequest[]> {
    const { data, error } = await this.supabase.rpc("get_open_cover_requests");
    if (error) throw error;
    return openCoverRequests.parse(data);
  }

  /**
   * File "I can't make this session" for a session the caller is expected at.
   *
   * The role being covered is resolved server-side — the caller's assignment
   * role, or the role stored on the cover they themselves hold — so a sub
   * asking for a sub needs no extra argument.
   */
  async requestCover(args: {
    groupId: string;
    sessionDate: string;
    reason: CoverReason;
    reasonNote?: string;
  }): Promise<CoverRequestDocument> {
    const note = args.reasonNote?.trim() ?? "";
    const { data, error } = await this.supabase.rpc("request_session_cover", {
  ...
  async withdrawRequest(requestId: string): Promise<CoverRequestDocument> {
  ...
  /**
   * Approve one offer, making that gedu the cover. The other offers are left
   * alone — "not selected" is derived from the request being covered by
   * somebody else, and an offerer never learns who else offered.
   */
  async approveOffer(offerId: string): Promise<CoverRequestDocument> {
    const { data, error } = await this.supabase.rpc(
   ...
   * beside the approval flow: an off-platform cover that has already happened
   * has to be recordable, because gedu invoicing reads these rows. Where the
   * absent gedu already has a request, this covers it; where they have none,
   * one is filed on their behalf already covered; where they are already
   * covered, the cover is re-pointed at the new sub in one action.
   *
  ...
  async setCover(args: {
    ...
    const note = args.reasonNote?.trim() ?? "";
    const { data, error } = await this.supabase.rpc("set_session_cover", {
      p_group_id: args.groupId,
      p_session_date: args.sessionDate,
   ...
   */
  async clearCover(requestId: string): Promise<CoverRequestDocument> {
    const { data, error } = await this.supabase.rpc("clear_session_cover", {
      p_request_id: requestId,
    });
    if (error) throw error;
    return coverRequestDocument.parse(data);
   ...
   * request, covered or not, and unwinds the substitution with it rather than
