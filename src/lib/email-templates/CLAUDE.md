...
- **`product-confirmation-invitation.ts`** — the signup mail's half of the calendar work: a product's schedule as one `InvitationInput`, plus the sentences the mail and the entry both state. It lives here rather than under `src/lib/calendar-invitations/` because it knows what a club is, which that module deliberately does not — see its `CLAUDE.md`. Pure, `now` is an argument, and it runs in a browser bundle like everything else on the registry's path.
