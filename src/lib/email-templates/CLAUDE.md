...
- It renders the schedule in the **product's** zone and names that zone in a sentence,
  because a mail has no viewer zone — the page reads one from the signed-in profile and
  decorates a time-bearing line with the short abbrev instead. Same formatter, one extra
  line.
