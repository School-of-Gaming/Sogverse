...
- **`layout.ts`** — `wrapInLayout({ title, content, locale, t })`: the branded dark-theme HTML shell every email uses. Table-based, inline CSS. `content` is your inner HTML. It emits the header's brand mark, above the lockup and never instead of it, and it carries the one stylesheet a mail has (see "Images" and "Layout gotchas" below). **The inline layout is the phone's and the card is what a wide viewport adds**: the base shell is a single 16px side gutter with the content sitting straight on the dark ground, and the one media query draws the panel — its pinned fill, its border, its corner, its 32px padding, the wider outer gutters and a little more air under the header — above the width that can afford them. Every class name the shell emits is declared in one place in that file — the gutter cell and the panel cell are the two that carry one — and the one selector inside the query that is not the shell's own comes from the module that emits the cells it matches, so it cannot drift away from that markup.
...
- **`blocks.ts`** — the composed blocks a template reaches for rather than builds. Mostly the ones a mail that sends the reader somewhere needs: `ctaButton()` (`primary` brand orange, `secondary` brand purple, `outline` bordered), `ctaButtonRow()` (two non-primary buttons side by side in a fixed 50/50 split, compact padding so both fit a mobile-width cell), `inlineLink()`, `bulletList()`, `sectionLabel()`. Every `href` they take is embedded unescaped, so they take app-generated URLs and nothing else. A mail with two buttons has one action it is actually asking for — that one is primary, the other outlined. Plus `calloutPanel({ label, paragraphs })`, for a mail with something to say about *itself*: a box with no fill at all whose border carries the status at full value (`STATUS.info`), its uppercase label in that same colour and its paragraphs in the ordinary body ink. The wash it used to carry went with every other tint — the coloured edge and the coloured label are what mark the panel as an aside, and a tinted ground was the same statement made a second time, more quietly. Its paragraphs are `foreground` throughout, which departs from the app's `Alert` in the tone it gives the description: the muted one is declined on purpose — the later sentence is usually the one answering the reader's actual worry, and it is not the line to quiet. And `factList(rows)`, the one label–value block every mail here states its facts in: a top rule, one row per fact, a hairline under every row including the last, an uppercase tracked label in a column as narrow as its own longest label, and the value in ordinary body ink. Labels and values both go in as composed HTML and neither is escaped there, so a value off a row is escaped (and, for an address, defused) by its caller. It was the session report's private block until it became everyone's; the bordered, rounded box the other mails used is gone, because on a phone it spent a border, a radius and 16px of padding on each side of a column that had none of that to give — all of it on an outline around facts nobody was going to mistake for anything else. **There is one block and no variants of it**, staff mail included: staff read mail on phones too (owner's ruling), and the knob-per-difference helper that would keep two arrangements alive is the style engine this directory has already declined once. There is no label-width option for the same reason the column sizes itself — a caller picking one was picking it for the English labels.
- **`session-photos.ts`** — the session report's photo column: the box arithmetic, one photo's toned well, and the one-per-row table they sit in. A module rather than a block in `blocks.ts` because one mail wants it and because a picture's box is arithmetic with rules of its own, which a template composing documents should not carry. The pairs it used to emit wanted a stacking rule in the stylesheet, and one photo per row is the same arrangement at every width — so nothing about the layout is shared with the shell any more. What the two files still agree about is one class name, exported here and used by the shell's existing wide-viewport rule to re-tone the well against the card it is then inside; the well is correct without that rule, which is why it is a re-tone rather than a layout the stylesheet holds up. See "Images" below for what the boxes are for.
...
- **`product-confirmation-invitation.ts`** — the signup mail's half of the calendar work: a product's schedule as one `InvitationInput`, plus the sentences the *document* states. It lives here rather than under `src/lib/calendar-invitations/` because it knows what a club is, which that module deliberately does not — see its `CLAUDE.md`. Pure, `now` is an argument, and it runs in a browser bundle like everything else on the registry's path. **What the entry's own `DESCRIPTION` states is a shorter list than the mail's, and the gap is the point**: where it happens, the My SOG link, the product's short description, and how to reach a human — in that order, because the entry is read on the way somewhere and the place and the link are what a parent opened it for, while the marketing sentence is the paragraph nobody opens a calendar for. Three things the mail states are deliberately not among them, each for its own reason. The schedule in words: a client draws the recurrence, the clock face and the zone out of the properties themselves and re-draws them when a later message moves the run, so a sentence beside them is the copy that goes wrong. The placement sentence: it is news about what happens next, and news goes stale where the mail it arrived in does not. And the opening sentence naming who is signed up for what: the entry's own title says it, and a reader is looking at that title while they read the notes — the mail opens with it because a mail arrives among fifty others and has to say what it is about, which an entry in a calendar does not. **The mail's schedule words are not composed here either, and that is the correction worth knowing**: they used to be, which gave one product's schedule two renderings — this module's sentences and the confirmation page's lines — that could disagree the moment either was corrected. The mail is that page's twin, so it now renders the page's lines through the page's own formatter (`src/lib/products/`), and this module composes the document alone.
...
## Family mail goes to the parent, and to a child who holds a mailbox

**Rule: we never write to a child alone. Every mail about a seat goes to the parent — the linked parent for a child's seat, the adult themselves for a seat held in their own name — and a seat with no parent contact produces no mail at all, whatever the child holds.** The parent's mail is the outcome: it is what a send counts, what a retry is about, and what a staff copy is a copy of.

**Rule: the child receives their own rendered copy when, and only when, their sign-in is their real email address.** A parent chooses how each child signs in; one of the three choices is the child's own email address, and the other two are a platform-internal handle no inbox answers. The mode is the whole test, it is re-checked on every send (a parent switching the child back takes the copy away with it), and a self seat is unchanged: there is no child to copy.

**Verification is deliberately not part of that test.** The address is typed by the parent and only proven when the child follows the verification link, and requiring the stamp was a special case this one role carried alone — it withheld the copy exactly where it is most useful, on a confirmation sent moments after the parent created the account, before the child had clicked anything. Children cannot be relied on to verify, and the parent is a recipient of every one of these mails regardless, so an unverified real address receives the copy.

**Rule: per recipient, never one mail with two addresses.** The parent and the child each get a separate send, rendered in that reader's locale and addressed to them — no CC, no BCC. A mail cannot be in two locales at once, and a copy is a different document: the child's framing sentence is written *to the child* where the parent's is written to the parent *about* the child, and the child's button goes to their own My SOG root, because role routing bounces a child off a `/parent` link.

**Rule: nothing only a parent can act on goes in a child's copy — above all, never a parent-only token.** The seat offer is the case that makes the rule: only the parent may accept or decline, and the links that do so carry a signed credential, so the child's copy is a separate builder with no answer buttons and no parameter that could hold one. It states the offer and that the parent decides. The signup confirmation drops its price row and its billing bullets for the same reason, and titles its summary card after a signup rather than after an order. Where a child's copy needs a variant, that variant belongs to the *template* (a flag or a sibling builder), so every caller renders the same child copy rather than deciding per surface what a child may see.

**Corollary: the test is who may act, not what looks parental — so the child's confirmation carries the calendar file.** A child with a mailbox has a calendar, and the sessions in it are theirs; withholding the `invite.ics` would be treating "the parent got one" as a reason the child may not. It is the **same calendar object** in both copies — one identifier, keyed on the seat rather than on the reader, so the two are one event seen by two people and a later update or cancellation is one message rather than two to remember. What differs is the **attendee**, which is that copy's own reader: a client offers the RSVP only when the attendee matches the mailbox it is reading, so the parent's document names the parent and the child's names the child. The entry's title follows the same reader rule the mail's sentences do — it names the participant only when the reader is not the participant, so a child's own entry is titled by the product alone, exactly as an adult's own seat is.

**Rule: a gedu never sees a child's address.** The roster resolves a child's seat to the parent's contact whatever the child's own sign-in is, and the family-mail rules above change nothing about that; a mail we send *to ourselves* about a child — a help request from a gamer account — keeps the parent as its Reply-To and may name the child's own address in the staff-facing note, so an admin can include both.

The decision of *who* lives in one shared resolver beside the Brevo wrapper, outside this directory (it reads profile rows, and builders here take no DB access); each builder decides *what* its child copy carries.
...
**Rule: Colors come from the shared constants, not Tailwind/hex literals.** Email HTML can't use semantic Tailwind classes, so import `BRAND` / `DARK_THEME` / `STATUS` from `@/lib/constants/colors` and interpolate them into inline styles. This is the email-context equivalent of the repo-wide "no hardcoded colors" rule.
...
| | app | email |
|---|---|---|
| Body font | the app face, loaded as a webfont | the mail face — the reader's own system sans, **and no webfont ever** |

**Rule: a mail is set in the mail face, and the mail face comes from the library through the constants module beside the palette — never as a family typed into a template.** The mail face is the reader's own system sans, declared once by the UI package and spent by this directory alone; the stack is emitted once on the shell and everything under it inherits. **No mail loads a webfont, and none ever will.** The clients most readers use — Gmail, Outlook, Yahoo — load none, so a declared family would reach a minority and the mail would be two designs rather than one; it is also a request to a third party on every open, which is an open-tracking beacon in a mail whose whole subject is trust; and Outlook on Windows answers a missing declared web font with Times New Roman rather than with the next family in the stack. **The Word engine gets its own resolvable stack**, from the same library face and derived beside the first: Outlook on Windows renders through Word, which does not walk a font-family list at all — it takes the first family and falls to Times New Roman where that family is not installed, rather than to the next entry — and the primary stack opens with names that exist only on Apple platforms. So the shell's head carries an `mso`-conditional block restating the same face in a form every entry of which resolves on Windows, on the elements the templates set text in; no other client reads it, no template writes it, and it loads nothing. The library's doc comment on the face holds the rest of the reasoning. Anything else you find where the mirror is imperfect, fix or list here — an unlisted difference reads as an accident, and this table is what makes it a decision.
...
## The signup confirmation is the confirmation page, in an inbox

**Rule: `product-confirmation.ts` and `/shop/confirmation` state the same *facts*, and
neither may lack a fact the other states.** A parent must be able to learn everything
about what they have just joined from either surface: what it is, who holds the seat,
when and where it runs, who it is for, what language it is in, what it costs, and what
happens next. A parent who paid on a phone and opened the mail on a laptop must not come
away knowing different things — which is what makes a fact stated on only one side a
defect on the silent side, wherever the silence looked defensible on its own terms.

**Each lays those facts out in its own medium's idiom, and that is not drift.** The page
is a page: two cards, a picture, a pair of buttons back into a shop the reader is still
standing in. The mail is a mail: one open-ruled list of rows, one button, because a boxed card
spends side padding a phone's column has not got to give. Adding a section to one
because the other has one is the mistake this rule exists to license refusing.

**Shared sentences should still read the same, and that is kept by care rather than by a
test.** The email translator is scoped to the `email` namespace and the page reads
`purchaseConfirmation`, `productDetail` and `productAudience`, so a sentence both state is
two strings in the message files and editing one is editing half of it — check the other
half when you touch either. The parity table that used to hold every shared key pair equal
...
all of them are the page's own rules, imported. A second rendering of a product's
schedule is exactly what this template used to carry, and it is what made the mail and
the page able to disagree about when a club meets.

**The four places the mail is allowed to differ**, each because the medium differs:

- It closes by inviting a reply, which is an affordance rather than a fact: a mail can be
  answered and a page cannot. The line is true because this send's Reply-To is the
  support inbox, so it goes wherever that does — a send that took a Reply-To of its own
  would have to take the sentence with it.
...
- It renders the schedule in the **product's** zone and **always** decorates the
  ...
  reads a viewer zone from the signed-in profile and decorates the same line with the same
  abbrev only when the two differ, since an abbrev beside times already in the reader's
  own zone is noise. Same formatter, same line, one option (`nameZone`). Not a sentence:
...
**Rule: an image's drawn size is arithmetic from dimensions the sender already holds, under a height budget — never from anything measured, and never a frozen pixel width.** The stored width and height are on the row beside the photo for exactly this. A picture is laid out fluid — full width of whatever column the client gives it, its height derived from its own ratio — under a `max-width` that is the height budget spent at that photo's ratio, and never above the stored width, since upscaling a stored JPEG only makes it soft. The height budget is the load-bearing half: without it a portrait reserves a screen and a half of empty grey for one picture, which is the render this rule exists to prevent. A fixed pixel width is the other failure and the one this replaced — a box sized for the narrowest client is a thumbnail everywhere else, and mail has no `object-fit`, so a picture stretched to a rounded box is a picture subtly the wrong shape.

**Rule: the reserved box is painted, and it is painted by a cell rather than by a background on the `<img>`.** A cell paints whether the image is blocked, deleted, still in flight, or stripped out of the document altogether — the same rectangle in all four cases — while an `<img>` that a client removed takes its own background with it. The fill is declared twice like every other background here, its tone comes from the palette, and the picture carries the cell's radius so the loaded and the blocked render have the same corners. **A well is a tone one step off the ground it sits on, and the shell has two grounds** — the bare ground on a phone and the card above the breakpoint — so the cell carries the card's tone inline and the shell's own wide-viewport rule swaps it for the darker ground, through a class exported by the module that emits the cells. That is a re-tone and not a layout the stylesheet is holding up: strip the block and a card-toned well on the dark ground is exactly the right render, because the dark ground is the only one a client that dropped the block will draw.

**Corollary: a reserved well is sized for the narrowest column, because a cell's height is a minimum.** Every client treats a table cell's `height` as a floor rather than as a fixed size, so the number to state is the smallest correct one — the height the picture has at the phone's content column, which is the mobile design floor less the shell's two gutters. A loaded picture on a wider column simply grows past it and the cell grows with it; a *blocked* one leaves a well that is the phone's height and the column's width, which is a shorter hole than the picture would have filled and never a taller one. Sizing every well for the widest column is the mistake it avoids: that reserves a hole bigger than the picture on every narrow render, which is the fault this whole design is about. The accepted cost is that on a wide client a landscape photo arriving pushes what is under it down; the phone, where these are read, holds its layout to the pixel.
...
- **The header carries no gradient at all: it is the shell's own ground, with a solid violet rule under the lockup.** A class-based `.hero-gradient` used to sit on both `<body>` and the outer `<table>`, because Gmail Android rewrites an inline `linear-gradient()` into `url(linear-gradient(...))` and breaks it. Nothing blends there now — the rule is a 6px filled `td`, pinned like every other background, which is the most robust construct email has. The violet is the mail's one second colour, and it is spent deliberately: a mail is a parent-tier surface, whose colour budget is amber alone unless an intent is stated beside the site that spends it, and the intent here is the signature pair — amber and violet are the brand's own lockup, and the header is where a mail says who it is from. Nothing below the header spends a second colour.
- **Brand-colored text uses a Gmail-only `background-clip:text` rule** behind the `u + .body` selector (which matches only Gmail's render wrapper). Gmail Android dark mode shifts the CSS `color` property but preserves gradient/background values, so brand text is recolored via a flat gradient + `background-clip:text` for Gmail and via plain inline `color` everywhere else. The header lockup and `styledName` emit the matching `brand-act` class — keep the class names in sync between `utils.ts`, `layout.ts` and the `<style>` block.
...
- **The one media query lives here, and it is the exception that proves the rule below it.** A media query is the one thing that cannot be written inline, which means the shell's `<style>` is not its preferred home but its only one. What keeps that honest is the direction it runs in: **the base layout is the phone and the query adds the card**, so stripping the block leaves a complete, correct mail — a plain column on the ground, every colour, every rule and every word intact — rather than a mail waiting for a stylesheet. The clients that drop the block are not hypothetical (Outlook on Windows; the Gmail app signed in to a non-Google account), and a card in the base with a query that removed it would have failed exactly where it matters, on the phone most of these are read on. The breakpoint is arithmetic rather than a round number: the content column plus one wide gutter a side. A second query is a second thing that has to be true in a client that ignores it — the photo grid used to want one and no longer does — so the count is one, and a layout that needs a rule here has to earn it by being wrong without one.
...
**So the two brand colours are not interchangeable, and world's one usable shape is as a fill under white** — which is what the app already says (`--color-world-foreground: #FFFFFF`), and what `blocks.ts` now spells as `variant: "secondary"`. Transplanting act's recipe — a brand fill under the near-black label — gives 2.9:1 and fails, because orange is a light colour and purple is a dark one. Copying a working button and changing only its fill is the specific mistake to avoid, and the variants exist so that nobody has to: the fill and its foreground travel together and cannot be paired wrongly by a caller.
