...
## Family mail goes to the parent, and to a child who holds a verified mailbox
...
**Rule: the child receives their own rendered copy when, and only when, their sign-in is their real email address AND that address has been verified.** A parent chooses how each child signs in, and one of the choices is the child's own email; that address is *typed* by the parent and *proven* only when the child follows the verification link sent to it. Until then it is a string somebody typed, and a mistyped one belongs to a stranger — a session report or a seat offer about a named child must never land there. The sign-in mode alone is therefore never enough, the mode is re-checked on every send (a parent switching the child back takes the copy away with it), and a self seat is unchanged: there is no child to copy.
