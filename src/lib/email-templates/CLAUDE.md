...
**Rule: `product-confirmation.ts` mirrors `/shop/confirmation` section for section — the
heading and its opening sentence, the order summary, the "Good to know" facts, "what
happens next", and the page's two buttons — and every sentence they share is one
sentence.** A parent who paid on a phone and opened the mail on a laptop must not be told
two stories, and "the same copy" is not a thing prose can hold: the email translator is
scoped to the `email` namespace and the page reads `purchaseConfirmation`,
`productDetail` and `productAudience`, so the strings are genuine duplicates. What holds
them equal is a **parity table** in the template's unit test — every shared key pair,
checked in every locale, with the two sides' placeholder names normalised (`{gamer}` /
`{product}` against `{participantName}` / `{productName}`). Edit one side and the build
fails; add a shared sentence and add its pair.
