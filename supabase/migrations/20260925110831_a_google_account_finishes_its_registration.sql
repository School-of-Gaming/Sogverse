...
--    the service-role writers each column now has: for the stamp, the
--    registration-completion routes and the OAuth callback when Google signs
--    an as-yet-unverified account in under its own address; for utm_source,
--    the registration-completion route. utm_medium and utm_campaign defer to
--    utm_source's rules, so theirs stand.
...
-- 7. forfeit_password(p_user_id) sets one auth user's password to NULL, which
--    is how an account created through Google holds none. The OAuth callback
--    calls it when a Google identity reporting the account's own address
--    verified signs into an account whose address was never proven: password
--    registration does not prove the address, so the password may be a
--    squatter's, and revoking sessions alone would leave them able to sign
--    straight back in. The account becomes Google-only; a password comes back
--    only through the reset email, which reaches the proven owner. The
--    `email` identity row stays: it carries no credential, password sign-in
--    never reads it, and the reset flow keeps working with it in place.
--    service_role only.
--
-- WHAT DID NOT CHANGE
--
...
-- forfeit_password: the Google claim takes the account's password away.
...
REVOKE ALL ON FUNCTION public.forfeit_password(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.forfeit_password(uuid) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.forfeit_password(uuid) TO service_role;
