-- The cover queue says WHEN the session is, not only which day.
--
-- WHY
--
-- 00260 gave the admin dashboard its fifth member, `cover_requests`, and shipped
...
-- draws.
--
-- The missing input is the SLOTS. The plan's rule for every cover surface is
-- that the database emits the date plus the product's slots and timezone and the
-- client computes the instants — which is what `get_open_cover_requests` (the
...
--
-- The body below is 00260's verbatim, with one key added to one jsonb object and
...
-- Extended rather than restated, for the reason 00260 gives where it does the
...
BEGIN
  ...
  EXECUTE format(
    ...
    v_existing ||
    ' Since 00264 each cover request''s product shell additionally carries'
    ...
    ' on every cover surface, exactly as both session feeds do. They ride on the'
...
-- tests/db/session-cover.test.ts parses this document's real output through the
...
BEGIN
  ...
  -- schedule set's and, new here, the cover queue's. One would mean this
