...
-- WHY
--
-- 00272 made "every gate on gedu_group_assignments carries a substitution branch or is
...
--
-- So the check's policy half lived only inside 00272's own end-of-migration
...
--
-- 00272 is already applied to staging, and an applied migration is never edited
...
-- 00272's question, for the reason every other `_list_*` helper is shaped that
...
DECLARE
  v_count integer;
BEGIN
  -- It has to see the policy 00272 rewrote, or the completeness check it exists
  ...
  SELECT count(*) INTO v_count
   ...
   WHERE p.table_name = 'product_groups'
     ...
     AND p.expression LIKE '%gedu_substitutes_group%'
