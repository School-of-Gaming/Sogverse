...
- [ ] **Consolidate existing preview scenes and UI Components demos under the demo-economy rules.** Both surfaces sprawled before the rules were codified (root `CLAUDE.md`: "one demo per piece, states side by side" inside the style guide's when-to-add-a-demo rule; "one scenario holds everything that can coexist, a second only for mutual exclusion" inside the scene-mocks-the-whole-page rule; description budget inside the registry rule): too many near-duplicate scenarios, per-state entries that could sit side by side in one render, and prose preambles narrating what the render should show by itself. Pass over `src/components/preview/` and `/admin/ui-components`: merge scenarios whose states are not mutually exclusive, fold per-state demos into one section rendering the states together, and cut descriptions to a line of why (or nothing). A judgement pass, not mechanical — each merge has to confirm the states really can coexist in one render.
