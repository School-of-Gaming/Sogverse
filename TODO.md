...
- [ ] **Provisional labels for cleanup-generated names.** The fold-in script labelled each entry after the earliest product that used it, which is a plausible name for a picture 22 products share and a wrong one. Marking those as provisional would let the relabelling pass find them instead of relying on someone remembering which they were.
...
- [ ] **The sheet lazy-mount fix.** The gedu and participant pickers fetch on page mount rather than on open, which is the same class of waste a closed dialog avoids by mounting its reads inside itself.
- [ ] **A `Dialog` scroll lock with gutter compensation, if a tall dialog ever needs one.** Not added for the catalogue: a lock changes every dialog in the app and shifts the page under the scrollbar, and the grid's own scroll container was enough. If one is ever added it has to compensate for the scrollbar's width or it breaks the layout rule for every caller at once.
- [ ] **`og:image` width and height, once dimensions are known.** No column records them today; the rendition step is where they would come from for free.
- [ ] **Drop `p_image_path` from the create and update product RPCs.** The routes stopped passing it — the trigger overwrites the RPC's assignment for a linked product and the parameter's default is already right for an unlinked one — so it is a parameter nothing supplies.
- [ ] **A second migration putting the design's invariants in the database, where they belong.** Three of them are currently kept by application code alone: a `CHECK (sha256 ~ '^[0-9a-f]{64}$')` so a hash column holds a hash; a shape constraint tying `path` to `sha256 || '.' || ext` so the object key cannot drift from the bytes it names; and, once no legacy path exists anywhere, a foreign key from `products.image_path` to `product_images(path)` so Postgres owns "a served path is a catalogue path" instead of a trigger being trusted to. The FK is the one with a precondition — it cannot be added while any product still sits on an un-catalogued path — which is why it waits on the cleanup being finished in both environments rather than on someone finding time.
