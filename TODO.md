...
- [ ] **`computeSessionWindow` still back-steps a week with flat `now - 7×24h` arithmetic.** `src/lib/session-schedule.ts` finds its previous-occurrence search point by subtracting fixed 168 hours, so on a DST transition week the point lands an hour off the local week boundary and the prev-window check can misjudge an in-progress session for up to an hour — the same 24-hour-day assumption fixed in `getNextSessionStart` (see the anti-pattern in root `CLAUDE.md`), with a narrower blast radius: the voice window's open/closed state. Its own comment even claims the lookup is DST-safe; only the *forward* half is. Fix the way `src/lib/session-occurrence.ts` does — step back one calendar week in the schedule zone (its private `stepBackOneWeek` is the exact shape; share it rather than duplicating) — plus a transition-week regression test.
