...
- [ ] **Finnish has three words for "professional" in front of `pelikasvattaja`, and the commonest one is ungrammatical.** The role name itself is settled — every locale now uses one word for it, ruled by native speakers and written up in `src/i18n/CLAUDE.md`. What that sweep deliberately did not touch is the *qualifier*: `messages/fi.json` carries `ammattimai-` (5 uses, incl. the metadata description), `ammattitaito-` (2) and `ammattilai-` (4, incl. the homepage FAQ) with no pattern. Two things to settle together, both needing a Finnish speaker: **which adjective**, given `ammattimainen` (professional in manner) and `ammattitaitoinen` (professionally skilled) make different claims about staff; and **the FAQ's grammar** — `ammattilaiset pelikasvattajat` is a plural noun-noun apposition where standard written Finnish wants an adjective, or the compound `ammattilaispelikasvattajat`. Worth asking whether the qualifier earns its place at all: the product tag copy the owner wrote uses a bare `pelikasvattaja` and reads fine.
