...
- [ ] **Decide the shape against the gedu-photo feature's real inputs, not now.** That feature (next item) is the second consumer, and a rendition step designed for one consumer is machinery for a requirement nobody has stated. It brings HEIC, a feed-sized rendition, retention and metadata stripping — all of which change what "derive a rendition" has to mean.
...
Owner-proposed, not designed. Gedus want to put pictures in a session report — taken on a phone, at a session, and read by the child's parent. It is the second consumer of stored images, and it is the one that decides what the rendition step above has to be, so the two are worked out together. What the catalogue work already learned and this feature should inherit:

- [ ] **Content-addressed originals are the masters.** Name the object by the sha256 of its bytes, never overwrite, and let "we already have these bytes" be an answer rather than a duplicate. It is what makes a URL's bytes immutable, and a phone that retries an upload gets the same object instead of a second copy.
- [ ] **A feed-sized rendition, derived from the master.** A session feed shows many photos small; nobody needs a 12 MP original to render one, and the original is still there when a bigger size is wanted later.
- [ ] **HEIC arrives whether or not it is wanted.** iPhones hand over HEIC, which browsers do not display — so this feature cannot be "store what arrives" the way the product catalogue is. Either transcode on the way in or reject it with copy a gedu can act on; decide deliberately, because a silently broken picture in a report is worse than a refusal.
- [ ] **Strip metadata — this is safeguarding, not tidiness.** A phone photo carries GPS coordinates and a capture time, and a report about a child is the last place to forward them.
