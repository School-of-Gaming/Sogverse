...
- [ ] **Parent home location (future feature) — the persistence decision needs re-taking.** The old note said to persist the choice as a catalog reference (`country_code` + `external_code`) on the profile, because the picker browsed static catalogs and had never seen a row. It browses `locations` rows now and hands back the row plus its ancestors, so a code pair is no longer the natural thing to hold — a `location_id` is, and it is already unambiguous where a code alone was not. What has *not* changed is the question underneath: a parent's home location is profile data rather than shared reference data, so whether it wants a real FK or a loose reference is still a call to make when the feature is built.
