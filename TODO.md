...
- [ ] **The browser hook over "one country's municipalities" lost its last caller, and the TypeScript search fold lost its last production one.** Both fell out of the same change: the product form's municipality field became a tree dialog, so nothing in the browser fetches Finland's kuntaa to pick from any more, and nothing narrows an in-memory set of places. Three things are now test-only or caller-less and they are deliberately *not* deleted, for the same reason as the flat-sites read below — what they hold up is worth re-homing rather than dropping. (a) `useMunicipalitiesByCountry` + its `locationKeys.municipalitiesByCountry` entry and its `initialData` option (`src/services/locations/`): the **service method** underneath is still very much alive, server-side, on all three `/schools` routes, so only the hook and its cache key are dead. (b) `foldForSearch` (`src/lib/locations/search-fold.ts`): its unit test and the DB agreement test are what pin the browser fold to the SQL one, and that DB test is named as the `scopeTest` for three functions in the authorization spine — so removing the fold means re-homing or retiring those entries too, in a change only CI can exercise. (c) `src/services/locations/CLAUDE.md`'s "the fold exists twice" rule is honest about (b) today, and should be simplified if the fold goes. Decide: delete all three and rework the spine entries, or leave the fold as the ready-made answer for the next surface that narrows a set client-side (the `/schools` directory is the obvious candidate — it renders a whole country's municipalities with no search box at all).
