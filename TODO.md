...
- [ ] **Finish retiring the old public storefront routes (`/clubs`, `/camps`, `/events` + `/[id]`) once `/shop` is live.** They're temporarily redirected to `/shop` via a `redirects()` block in `next.config.ts` (signed-in and signed-out users alike) so the source can stay in the tree for reference. When `/shop` ships:
  - Delete the six page files: `src/app/(public)/{clubs,camps,events}/page.tsx` and `.../[id]/page.tsx`, plus any `src/components/public/products-v2/*` they were the sole callers of.
  - Repoint or remove the remaining `ROUTES.clubs` UI links (home CTA in `src/app/(public)/page.tsx`) and the `/clubs|/camps|/events` prefixes in **both** `src/hooks/use-auth-redirect.ts` and `src/proxy.ts` (`PUBLIC_ROUTES` still lists `ROUTES.clubs`/`camps`/`events` — harmless while they redirect, but should be dropped at full delete). The old parent `parent/gamers/page.tsx` links are already gone — that page was deleted.
  - Remove the temporary `redirects()` block from `next.config.ts`.
  - Add `/shop` to `src/app/sitemap.ts` — the old `/clubs` entry was removed when these routes were deprecated, and `/shop` wasn't added in its place because it's still a placeholder.

- [ ] **Finish ripping out the v1 product/groups stack once v2 reaches parity.** The admin/gedu groups *viewing* pages and their wrappers are already gone (see commit `1fbad34`). What's still on v1 and needs to come out wholesale at v2 cutover:
  - **v1 product page + edit:** `src/app/(dashboard)/admin/products/[id]/page.tsx`, `.../[id]/edit/page.tsx`, `.../add/page.tsx`, plus the `ROUTES.admin.products*` and `ROUTES.admin.product*` constants in `src/lib/constants/routes.ts`. Admin nav already points at v2 (`/admin/{consumer-clubs,municipality-clubs,camps,events}`), so the v1 product surface has no entry point — it just compiles.
  - **v1 group editor UI:** `src/components/admin/gedu-groups-card.tsx`, `group-card.tsx`, `commit-bar.tsx`, `commit-flow-dialog.tsx`, `commit-flow-parts.tsx`, `gedu-picker-dialog.tsx`, and `src/hooks/use-group-editor.ts`. All only used by the v1 product page above.
  - **v1 groups apply route:** `src/app/api/admin/products/[id]/groups/apply/route.ts` (SSE-streamed, sends emails for every change). Also the email templates it uses in `src/lib/email-templates/group-changes.ts` if v2 doesn't reuse them.
  - **v1 viewing-side that's still wired:** `parent/groups/[id]/`, `gamer/groups/`, `gamer/groups/[id]/` all still call `useMyGroups` from `src/services/groups/` against v1 tables (`product_groups`, `group_enrollments`). These can't go until v2 has an equivalent RPC for gamer/customer roles (RLS on `product_groups_v2` is in place; the service-layer + RPC isn't). The role-specific renderers behind those routes go with them — notably `src/components/customer/CustomerGroupDetailContent.tsx` (the parent-side wrapper over `GroupDetailContent`); it's *not* in the shared-layer bullet below because it's customer-only, and its only consumer is `parent/groups/[id]/page.tsx`, so it would orphan silently if that route were deleted on its own. (Note: `src/app/(dashboard)/parent/gamers/` is **not** v1-groups — the index page that called `useMyGroups` was deleted, and the surviving `parent/gamers/[id]/page.tsx` is the live gamer-settings page reached via `MyGamersGrid` on `/parent`. Leave it.)
    - When `parent/groups/[id]/` goes, also delete the now-dead `ROUTES.customer.group(...)` URL builder (`src/lib/constants/routes.ts`) — this PR removed its last caller, so it has zero usages — and repoint or drop the two standalone `ROUTES.customer.gamers` references that currently dead-end at the deleted `/parent/gamers` index (the `redirect()` in `parent/groups/[id]/page.tsx` and the `backHref` in `CustomerGroupDetailContent.tsx`). Reachable only by hand-typing the URL today, so low priority, but they go with the route.
  - **Shared component layer** that today straddles v1 and v2 callers: `src/components/groups/GroupsListContent.tsx`, `GroupDetailContent.tsx`, `src/components/ui/{group-card,lounge-card,join-button,group-voice-status}.tsx`, `src/hooks/use-groups-page.ts`, the entire `src/services/groups/` module. Some of this may stay (renamed) for v2 viewing pages — decide per-component at cutover.
  - **DB:** `product_groups`, `group_enrollments`, `enrollment_charges` tables; `commit_group_changes`, `get_product_groups_with_details`, `get_my_groups`, `enroll_gamer_in_group`, `unenroll_gamer`, `get_enrollment_groups` RPCs; the cross-table uniqueness trigger. Drop the matching entries from `tests/db/access-control.test.ts` allowlists.
  - **Tests:** `tests/integration/api/groups-apply.test.ts`, `tests/unit/groups-service-gedu.test.ts`, `tests/db/my-groups.test.ts`, `tests/unit/email-templates/group-changes.test.ts` (if v2 doesn't reuse the templates), `tests/db/access-control.test.ts` entries.
  - **i18n:** keys still kept solely for v1 — re-audit at cutover. Candidates: `admin.groups.*` (used only by v1 `gedu-groups-card`), `groups.currentGedu`/`assignedGedu` (used only by `gedu-picker-dialog`), `groups.{title,backToGroups,groupNotFound,live,gamerCount,noGamersEnrolled,age,unenrollGamer,noGroupsYet,nextSession,startsIn,sessionInProgress,everyDay,join,alwaysOpen,padlet,myGroupsHeading,myGroupsSubheading,myGroupsEmpty}` (used by the shared `GroupsListContent`/`GroupDetailContent`/`GroupCard`/`LoungeCard` — keep only what v2 viewing UI ends up consuming).

  **Blockers before this is safe to do (real v2 parity gaps as of 2026-05-27):**
  - Viewing pages have no v2 data path (no `get_my_groups_v2`-equivalent service + RPC for gamer/customer).
  - v2 apply route sends no emails on commit (`src/app/api/admin/products-v2/[id]/groups/apply/route.ts` has a comment saying so explicitly).
  - v2 group deletion silently unassigns enrolled gamers (`participations_v2.group_id` has `ON DELETE SET NULL`); v1 has `ON DELETE RESTRICT`. Add a warning segment to `commit-summary-dialog.tsx` or restore the restrict.
  - No v2 equivalent of `VisibilityWarningBanner` (warn when product is visible with zero groups). v2 RPC also doesn't auto-hide on last-group-delete — confirm intentional.

  Daily.co pre-creation has *also* been ripped out of the v1 route (commit `<this PR>`) — it was the one piece of v1 cleanup that didn't need v2 parity first because v2 deliberately never had it.
...
- [ ] **Replace hand-written row types + `as` casts on embedded `.from().select()` queries with `QueryData` inference.** Reference example: `getParticipationsForGamers` in `src/services/participations/participations.service.ts` — the query lives in a standalone builder and the row type is derived via `QueryData<ReturnType<typeof builder>>[number]` (with `!inner` on the NOT-NULL FK embed to recover the non-null type), so the select string and the type can't drift and there's no cast. See CLAUDE.md § "Verify generated nullability" for the rule. Sites still on the old hand-write-and-cast pattern, in rough ROI order:
  - `src/services/products-v2/products-v2.service.ts:218` (`listVisibleByType`), `:266` (gedu-assigned, same shape), `:367` (`getByIdForAdmin`) — direct-return embedded selects with a cast, exactly the reference shape. Highest value, but `ProductV2BrowseRow`/`ProductV2AdminDetailRow` are large hand-written types — check whether either *intentionally* tightens nullability (the `Omit` + intersection pattern) before swapping, so the inferred type doesn't silently re-widen a column.
  ...
  - `src/services/participations/participations.service.ts:280`/`:361` (`getMyParticipations`/`getMyUpcomingSessions`) — these run a mapper, so the *output* type (`MyParticipationRow`/`MyUpcomingSessionRow`) is legitimate; only the raw *input* cast (`RawMyParticipationRow`/`RawMyUpcomingSessionRow`) is removable. Lowest value.
...
- [ ] **Split subsystem-specific rules out of root `CLAUDE.md` into nested CLAUDE.md files.** Root CLAUDE.md is ~9k tokens and loaded on every turn. Claude Code lazily loads `CLAUDE.md` from any ancestor directory of a file being read/edited, so subsystem rules can live next to the code they govern — they cost zero tokens until that subtree is touched, and become *more* prominent (loaded alongside the code) when it is. Candidates, in rough ROI order:
  ...
  - **`src/services/groups/CLAUDE.md`** — only worth adding if `src/services/groups/` survives the v1 → v2 cutover; today its rules apply solely to v1 code that's scheduled for deletion.
