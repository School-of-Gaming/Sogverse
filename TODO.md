...
- [ ] **Staging has been hand-edited outside migrations; decide whether to repair it.** Established 2026-07-29 by tracing the prod↔staging diff case by case against migration source. **Prod matched migration history in every instance; staging did not**: `get_user_role`'s body is reformatted, function-body comments are stripped from the gedu self-registration RPC and the feedback rate limiter, two `COMMENT ON` statements (`profiles`, `parent_gamer`) exist that appear in no migration, and `profiles.currency` was dropped and re-added so its ordinal moved.
  - **The snapshot is no longer affected** — `schema.sql` is now built from migrations in CI, so staging's drift can't reach the committed file. This item is only about staging itself.
  - Nothing here is behavioural: comments, whitespace and column order. The argument for repairing it is that staging is where migrations are rehearsed, so the further it drifts from migration history the less that rehearsal proves. The argument against is that every fix is a `CREATE OR REPLACE` of a body that already behaves correctly, and the ordinal can't be fixed without rebuilding the table.
  - If repaired, do it as a migration (idempotent on prod, which is already correct) rather than by hand — hand-editing staging is what caused this.
