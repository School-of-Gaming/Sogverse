...
- [ ] **Migrate remaining external-redirect call sites to `useExternalRedirect`.** The hook (`src/hooks/use-external-redirect.ts`) was added during the v2-stripe-participations branch to fix the CTA flicker on the product detail signup panel: `mutation.isPending` flips false the instant `onSuccess` returns, but `window.location.href = ...` schedules the unload asynchronously, so the button re-enables for one frame and a fast user can double-click. Three older call sites still hand-roll the same pattern with various amounts of safety:
  - `src/components/tokens/token-purchase-section.tsx` — uses a manual `loadingPriceId` state with a comment ("Don't clear loading state...") that's correct in spirit but bespoke
  - `src/app/(dashboard)/parent/billing/page.tsx` — naked `window.location.href = data.url` in a button handler with no flag at all (flicker risk identical to the one we just fixed)
  - `src/app/(public)/checkout/page.tsx` — same pattern as billing page
  Each should swap to `useExternalRedirect`. Trivial mechanical change, deferred so the hook lands with one consumer first. See CLAUDE.md "Loading & Disabled State" for the rule.
