...
- [ ] **Relabel the entries the one-time fold-in named.** All 44 prod entries took the name of the earliest product that used them, which is a plausible label for a picture one product wears and a wrong one for a picture 22 share: eight distinct pictures currently read "Minecraft: Cozy Adventures" and one reads "Testing Club (do not buy)". It is a pass through the catalogue dialog's rename control and needs nothing built.
- [ ] **Provisional labels, so a pass like that can find its own work.** The fold-in's names are indistinguishable from names an admin chose, so the relabel pass above relies on someone remembering which were which. Marking a generated label as provisional would let the dialog surface them instead.
