...
- [ ] **Per-participant volume slider — dead wiring kept, UI removed.** The discrete-zone voice redesign dropped the per-participant volume slider from `ParticipantList`/`ParticipantRow` to reclaim space. The `element.volume` multiplier plumbing stays in `src/components/voice/hooks/use-audio-pipeline.ts` (the `base` term in `zoneVolume`, read from `volumeMultipliersRef`, currently always 1.0 with no setter), so the slider can be re-enabled cheaply if users ask for it. Note the Chrome WebRTC limitation: it's capped at 100% (no above-unity gain) — see `docs/chrome-webrtc-volume-bug.md`. To restore: re-expose a `setParticipantVolume` action on the provider and a slider control.
