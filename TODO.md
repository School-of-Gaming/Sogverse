...
- [ ] **Reconsider `@vitejs/plugin-react@6` — Windows lockfile hazard for a cosmetic fix.** v6 (dev commit ffbd5f0) was bumped purely to silence a Vite deprecation warning about `esbuild`/`optimizeDeps.esbuildOptions` keys. v6 switched its internal bundler to rolldown, which pulls in `@rolldown/binding-wasm32-wasi` — a package that strictly pins nested `@emnapi/core@1.9.2` and `@emnapi/runtime@1.9.2`. Windows `npm install`, when it re-hoists the rolldown subtree during an incremental install, sometimes drops those nested pins; only the top-level `@emnapi/core@1.10.0` remains. Linux CI's `npm ci` then fails with `Missing: @emnapi/core@1.9.2 from lock file`. Already hit twice: dev commit 07b64f9 (clean regen), then feature/bedrock-portal-service commit edb6c71 (clean regen again after rebase + workspace add). The fix (clean `rm -rf node_modules package-lock.json && npm install`) is known but easy to forget, and the plugin is only used for Vitest test setup — not runtime code. Options: (a) add a pre-commit or CI preflight that runs `grep -c "binding-wasm32-wasi/node_modules/@emnapi" package-lock.json` and fails if ≠ 2; (b) downgrade to `@vitejs/plugin-react@5` and accept the deprecation warning; (c) move `npm install` for dep changes into WSL. (a) is the least invasive; (b) eliminates the hazard class entirely. Revisit when rolldown ships a release that doesn't pin emnapi so strictly, or when plugin-react's deprecation warning becomes a hard break.
