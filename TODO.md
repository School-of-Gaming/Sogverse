...
Safety copy states checkable mechanisms, never intentions, and only mechanisms verified true (root `CLAUDE.md`) — so a safeguard we want but do not have is a feature, never a sentence. These three came out of the 2026-08 brand audit as exactly that shape: the copy reaches for them, and they are not true enough to write down yet.

- [ ] **Nothing establishes what we actually hold about a child, so we cannot claim there is nothing about them to lose.** The shape we want is a **display name and an age bracket and nothing else** — no real name, no birth date, no real email — so that a breach, a subpoena or an acquisition has nothing about a child to take; the synthetic `<token>@gamer.sogverse.internal` address a gamer account already carries is that shape, and it is the model for the rest. Today a parent types a gamer name at creation with nothing steering them away from the child's real one, `create_gamer` takes a `date_of_birth` and a `gender`, and the audit could not establish what else accumulates around a gamer. **The fix is an audit and then a minimization:** enumerate every column and free-text field that can carry something identifying about a child, decide which of the two fields it collapses into, stop collecting or drop the rest — after which the copy can state what is held in one truthful sentence. **Collides with "Parent-Managed Gamer Profile Fields (DOB, Gender)" above,** which plans to let parents *edit* the birth date and gender that this item may delete; the two want deciding together, and the age-bracket question (a bracket held directly, versus an age derived from a stored DOB) is the crux.
