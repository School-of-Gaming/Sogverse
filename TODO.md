...
- [ ] **Deleting a product that has participations fails on the seat-count trigger — no delete path works while any child is signed up.** The `ON DELETE CASCADE` from `products` reaches `participations`, whose delete trigger (`trg_refresh_product_seat_counts`) calls `refresh_product_seat_counts(OLD.product_id)` — and that function unconditionally upserts a `product_seat_counts` row for a product the same cascade has already removed, violating the FK and rolling the whole delete back. Surfaced 2026-08-07 deleting the 19 staging webinar fixtures (worked around by deleting the participations in the same transaction *before* the products, so the refresh still found its product row). Any future admin product-delete flow inherits this. Fix shape: the refresh function should treat "product row absent" as "nothing to maintain" (skip the upsert, or delete the counts row), so a cascading product delete stops depending on child-table trigger ordering.
