...
- [ ] **Free-product signup CTA freezes after the first child when the parent has a second eligible child.** In the per-gamer signup picker (`src/components/public/products/signup-panel.tsx`), `handleSubmit` holds a local `committing` flag (the anti-double-click guard per CLAUDE.md "Loading & Disabled State") and only clears it on retry-able outcomes (`full`, error). For `free_confirmed` (free events/products) the flag is deliberately left set, on the now-stale assumption that the panel swaps to a different view — which it did before this branch (the old all-or-nothing `AlreadySignedUpPanel`). With the per-gamer picker the form stays mounted: after a free signup, the just-enrolled child flips to a disabled "Signed up" row and the picker auto-advances selection to the next eligible child — but `committing` stays `true`, so the CTA is stuck disabled showing "One moment…" and the parent can't enrol the second child without a full page reload. Single-child case is cosmetic only (button reads "One moment…" instead of settling). **Fix:** clear `committing` in `onSuccess` for `free_confirmed` (the success action targeted the now-locked child; re-enabling for a *different* selectable child is correct, and the single-child case disables via `selectedGamerId === null` anyway). (The old `subscribed` inline-add outcome that had the same shape is gone — every subscription is now a `redirect` to Stripe Checkout that unloads the page, so only the `free_confirmed` case remains.) Deferred from the `feat/shop-signup-picker-and-ended-filter` branch as out of scope.
