...
- [ ] **Consolidate hand-rolled badge pills onto the shared `<Badge>`.** ~11 places render a small status/label pill as a raw `<span>`/`<div>` (`rounded-full` + `px-` + `text-xs` + a bg tint) instead of `src/components/ui/badge.tsx`. **Root cause:** `<Badge>` only offers *solid* fills (`default/secondary/destructive/outline`) and no semantic `success`/`warning`/`info` tones, which is what every hand-rolled one reaches for. The soft tints the hand-rolled pills used to reach for (`bg-act/10`, `bg-destructive/20`) are no longer available: the theme adoption ruled that a colour is spent at its authored value, as a figure rather than as a wash, and the shape that replaced them is the one `ProductStatusChip` now carries — a neutral chip on a `border` edge with the **label** in the status colour. So the high-leverage fix is **semantic tones on `<Badge>` in that shape** (a `tone` prop, plus an `/admin/ui-components` demo), then fold the bespoke spans in. Strongest candidates, in order:
  - **The success variant is now owner-ruled, with a contrast requirement (2026-08-28):** `<Badge>` needs a proper `success` variant, and it should offer **better contrast than the hand-passed white-on-green** (`bg-success text-success-foreground`) that the admin user page's marketing-consents "Granted" badge ships with today. The soft tint that was the obvious candidate is out — no token is spent at an alpha step any more — so the two live options are the green as a **label** on a neutral chip (the lifecycle chip's shape) or a filled green under the ink @sog/ui has measured for it. That badge (`src/components/admin/user-marketing-consents-card.tsx`) becomes the first adopter when the variant lands.
  ...
  - **Semantic pills** — `schools-browse.tsx` `StatusPill` and the HTTP status-code badges in `docs/minecraft-api/page.tsx` (~6 spans wanting success/warning/destructive tones). Their tints are already gone; these unblock once `<Badge>` grows the toned variants above.
...
- [ ] **Split subsystem-specific rules out of root `CLAUDE.md` into nested CLAUDE.md files.** Root CLAUDE.md is ~9k tokens and loaded on every turn. Claude Code lazily loads `CLAUDE.md` from any ancestor directory of a file being read/edited, so subsystem rules can live next to the code they govern — they cost zero tokens until that subtree is touched, and become *more* prominent (loaded alongside the code) when it is. Candidates, in rough ROI order:
  ...
  **Stays at root** (genuinely cross-cutting): RBAC overview, auth/CSP rules, Layout & Scrolling, Loading & Disabled State, Button Order, Date/Time, Locale vs Spoken Language, what is left of Styling (the faces and headings rules — colour left for `packages/sog-ui/CLAUDE.md` with the theme adoption), Lint discipline, tech-stack overview, Service Layer Pattern (it's a navigation aid for new contributors more than a rule).
