...
- [ ] **Decide what to do with `/settings` and `/feedback` for parent/gamer roles.** The shared `<SettingsSectionContent />` and `<FeedbackSectionContent />` components are now rendered inside the parent/gamer dashboards as `#settings` and `#feedback` scroll sections. The standalone routes still exist (admin and gedu link to them from their sidebars). The intended end state — per the user's plan — is that parent/gamer have only `/parent#settings` / `/gamer#feedback` etc., and the standalone routes either redirect or stop existing for those roles. Two open subtasks:
  - For parent/gamer hitting `/settings` or `/feedback` directly (bookmark/old link), add a server-side redirect to `/parent#settings`, `/gamer#feedback`, etc. Probably in the route's `page.tsx` via `getUserWithProfile()` + `redirect(...)`.
  - Decide whether admin and gedu eventually adopt the same in-page-scroll dashboard pattern. If yes, the shared section components are already extracted — only the page composition would need to follow.
- [ ] **Add hash-anchor deep-link support to parent/gamer dashboards** (the same trick `/admin/ui-components` uses). The pill currently smooth-scrolls in response to clicks but does not honour the hash on initial load — landing on `/parent#feedback` from an external link won't auto-scroll to the feedback section. Out of scope when the unified navbar shipped, easy to add later.
