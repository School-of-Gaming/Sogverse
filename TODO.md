...
- [ ] **db-auth refactor: release-gated follow-ups (next dev→main release).** The refactor (merged to dev 2026-07-27, PRs #74–#77) leaves three items that can only happen around the production release, all recorded at the end of `docs/db-authorization-architecture.md` §5: **(1) before merging the release PR**, run the one read-only prod query confirming no participation row crosses the party/role columns (staging showed zero; the policy rewrite is safe regardless by the directional argument, this is belt-and-suspenders); **(2) after prod deploys**, ship the migration revoking `service_role` EXECUTE on the two legacy engine signatures (3-arg `join_waitlist`, 2-arg `submit_feedback`) — prod's *old* code calls them via the admin client until the release deploys, so revoking earlier breaks prod; **(3) after prod deploys**, diff the prod policy catalog against `schema.sql` for further out-of-band drift (the Phase 4 repair found two admin policies living outside migration history). Delete this item when all three are done.
