...
- [ ] **Wire the parent Sessions card's Join button to a real voice destination.** `src/lib/upcoming-sessions.ts` currently sets `voiceHref: "#"` for every session — `voiceIsOpen` still flips live on the buffer window, but clicking the active Join button is a no-op until the parent-side voice URL story is decided (own watch-mode page, or trampoline through a switch-to-gamer flow). Same `padletUrl ?? "#"` no-op pattern applies to the reports button when a product has no Padlet URL set.
