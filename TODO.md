...
- [ ] **db-auth refactor: release-gated follow-ups (next dev→main release).** The refactor (merged to dev 2026-07-27, PRs #74–#77) leaves items that can only happen after the production release, recorded at the end of `docs/db-authorization-architecture.md` §5:
  ...
  - [x] Revoke `service_role` EXECUTE on the two legacy engine signatures (3-arg `join_waitlist`, 2-arg `submit_feedback`) — done 2026-07-28 in migration `00126`, after the release deployed to prod. The three db tests that drove the engines through the admin client now go through the guarded wrappers, and each engine gained a pair of tests pinning that neither `authenticated` nor `service_role` can reach it directly.
  - [x] Diff the prod policy catalog against `schema.sql` — done 2026-07-28. **It found drift, and the drift is on staging, not prod** (the Phase 4 note had the direction backwards). See the next item, which is what remains.

- [ ] **Converge the `customer_profiles` / `gamer_profiles` policy names, and drop the duplicate admin policies `00125` created on prod.** Fallout from the db-auth Phase 4 conditional repair, found by the post-release prod diff:
  - Migration history (`00002`, `00003`) names these six policies in snake_case (`admin_full_access_customer_profiles`, `customers_read_own_customer_profile`, `admin_full_access_gamer_profiles`, `gamers_read_own_gamer_profile`, `gamers_update_own_gamer_profile`, `parents_read_linked_gamer_profiles`). **Prod matches migration history; staging is the database that was hand-renamed out of band** to quoted human-readable names (`"Customers can read own customer_profile"`, …).
  - `00125`'s repair block keys its `IF EXISTS` on the *quoted* admin name. Staging had it and took the ALTER branch; **prod did not and took the CREATE branch**, so prod now carries two admin policies per table — the original `admin_full_access_*` with a bare `is_admin()`, plus a new `"Admins can do everything on *"` with the InitPlan-wrapped `(SELECT is_admin())`. The migration's own comment claims "the hosted databases take the ALTER branch and are unaffected"; that is false for prod.
  - **Not a security hole** — both policies are permissive with the same admin predicate, so effective access is unchanged. What it costs is the Phase 4 invariant ("the admin half of every policy in the database is one shape") on those two tables, a per-row `is_admin()` call on the un-wrapped duplicate, and a `schema.sql` that does not describe prod.
  - Fix: one idempotent migration that drops the `"Admins can do everything on *"` duplicates, renames staging's six quoted policies back to the migration-history snake_case names, and leaves `admin_full_access_{customer,gamer}_profiles` InitPlan-wrapped. Must be written to converge from *either* starting state, since the two databases differ.
  - Separately: a bare `npx supabase gen types` now also emits a `graphql_public` block that the committed `database.types.ts` lacks — unrelated to any migration, but it means the documented regeneration command no longer round-trips. Worth pinning `--schema public` in `supabase/CLAUDE.md` or accepting the block.
...
- [ ] **Consolidate hand-rolled badge pills onto the shared `<Badge>`.** ~11 places render a small status/label pill as a raw `<span>`/`<div>` (`rounded-full` + `px-` + `text-xs` + a bg tint) instead of `src/components/ui/badge.tsx`. **Root cause:** `<Badge>` only offers *solid* fills (`default/secondary/destructive/outline`) — no soft tints (`bg-primary/10`, `bg-destructive/20`) and no semantic `success`/`warning`/`info` tones, which is exactly what every hand-rolled one reaches for. So the high-leverage fix is **add soft/semantic variants to `<Badge>`** (e.g. a `tone` prop or `soft-*` variants, plus an `/admin/ui-components` demo), then fold the bespoke spans in. Strongest candidates, in order:
  ...
  - **Lower priority / bespoke** (icons or `text-[10px]`/uppercase sizing — case-by-case): `product-rows.tsx:176` hidden flag, `location-picker.tsx:211` type label, `schools-browse.tsx` count pill, `voice/ZoneList.tsx:333` private-zone lock, `whatsapp/page.tsx:209` date divider. Also note a *good* existing pattern to consider folding into instead: `src/components/public/products/status-chip.tsx` (`StatusChip`, already has tones + sizes + optional icon). Own branch — bigger than a mechanical swap.
