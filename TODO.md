...
- [ ] **Delete the mobile fit-check card from the UI Components page once the add-gamer question is decided.** `/admin/ui-components` → "Game account" → "Mobile fit-check (temporary)" holds two fixed phone frames and a hand-built replica of the add-gamer dialog's field stack, built to answer one question: can both game username fields live in that dialog without a phone scrolling. **The measured answer is no** — 862px of content against roughly 567–607px usable on an iPhone SE class and 744–784px on a modern phone. One field alone (712px) clears a modern phone and still misses the SE. The two biggest line items are not the identity row: the gender buttons cost 160px because they stack until `sm`, and each game field spends 74px on its label and input before the row. It is marked temporary in the page itself and is a *replica*, not the real card — so it will drift the moment the real dialog changes, which is exactly why it should not outlive the decision. Delete the card, its `PhoneFrame`/`MockField`/`FIT_BUDGET` helpers, and this item together.

- [ ] **Migrate the production surfaces onto the unified game-account components, then delete the forked Minecraft/Roblox trees.** `src/components/game-account/` now holds one platform-parameterised set — descriptor registry, row, first-capture field, inline-editable row — demoed on `/admin/ui-components` under "Game account". Nothing in production uses it yet: `src/components/minecraft/` and `src/components/roblox/` still back the settings page, the parent gamer detail, gedu register, the admin badges, the voice room, and `GamerRosterRow`. Migration is per-surface and mostly mechanical, with three things to watch. **There is no compact badge and no size variant.** Every game identity is one height and carries its skin, so a caller currently rendering a skin-less one-line badge — the admin user rows, the gamer chips, the voice room — is choosing between the avatar-bearing row at that height or keeping its own bespoke line. That is a design call per surface, not a mechanical swap, and it is the part of this migration worth doing with the owner. **The status vocabulary changes**: the Minecraft components speak `idle`/`valid`/`invalid`, the unified set speaks `unknown`/`unverified`/`verified`/`checking`, and there is deliberately no `invalid` — a failed lookup is not a row state, so whichever surface ran the lookup keeps the error copy. **The Minecraft field's reveal animation goes**: it animates a grid row from `0fr` to `1fr` when a skin lands, which re-lays-out everything below it on an async schedule; the unified field reserves the slot from first paint instead. Once every caller has moved, delete both old directories, their unit tests, and the `minecraft.*` / `roblox.account.*` message namespaces (the `mcHeadsAttribution` string among them — the unified row does not render an attribution line, so decide deliberately whether that is a loss before dropping it). A colocated `src/components/game-account/CLAUDE.md` is worth writing in that same change, once the set has real callers to describe.
