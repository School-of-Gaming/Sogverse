...
`max_rows = 1000` (`supabase/config.toml`) is enforced by **truncating** the response, not by erroring, so an unbounded select is indistinguishable from a complete one. Confirmed against the hosted staging project on 2026-07-30: an anon select on `locations` (~35k rows) returns exactly 1000 rows and no error. The shared `walkPages` primitive (`src/lib/supabase/paging.ts`) walks `.range()` until a page comes back short and cross-checks what it collected against `count: "exact"`; the locations reads and the `/admin/users` reads (profiles, parent↔gamer links, gedu verification) go through it. The remaining list reads across `src/services/` are unbounded, so each one still has a silent correctness cliff at 1000 rows. The ones that matter:
