...
- [ ] **Two locales disagree with themselves about their own house terms.** `messages/fi.json` calls game educators `ammattilaiset pelikasvattajat` in the homepage FAQ (`whatIsSogverse`) but `ammattimaisten pelikasvattajien` in the metadata description — and the FAQ form is a plural noun-noun apposition where standard written Finnish wants the adjective `ammattimaiset` (or the compound `ammattilaispelikasvattajat`). `messages/sv.json` splits the same way on the role itself: `spelpedagoger` in the FAQ and everywhere else, versus `spelfostrare` in the metadata description, so the description is the outlier. These are the words for the people who actually deliver the product, in the two locales with real customers, and the metadata description is what a parent reads in a search result — so the split is visible as two different job titles between the snippet and the page it opens. Pick one term per locale and make the description match the body; the FAQ copy was rewritten during the brand-lockup pass and deliberately carried these through unchanged rather than widening that commit.
