...
**The catalogue is also massively duplicated, and dedup composes with the re-encode (prod audit, 2026-08-19).** The bucket held 110 objects / 124 MB; the 104 objects products reference collapse to just **44 distinct images** — 60 are byte-identical copies (verified by MD5 of downloaded bytes, not just eTag), because the upload route mints a fresh `randomUUID()` path per upload, so an admin re-uploading the same picture always creates a new object. The two worst groups sit on live shop products: one 996 kB PNG stored 23 times and one 209 kB PNG stored 22 times — 43 of the 82 visible products wear just two pictures via 43 distinct URLs. Six more objects (9.1 MB) are orphans nothing references. The URL identity matters more than the storage: browser, Vercel optimizer, and unfurl-scraper caches all key on URL, so distinct URLs for identical bytes mean 43 downloads/transformations/origin-fetches where a shared URL would mean 2. Caveat: byte-identity is the floor — visually-identical re-exports through different tools won't match, and that's fine; exact-match is where nearly all the value is.

- [ ] Re-encode at upload in the two admin product routes (cap ~1600px wide, WebP q≈80 — ~150–250 kB per image)
- [ ] **Content-address the object path**: name each upload by the hash of the *encoded* bytes (`<sha256>.webp`) instead of `randomUUID()`. Identical uploads land on the same path by construction, which makes the immutable-bytes contract true by construction too; the existing `upsert: false` "already exists" response becomes the dedup hit — reuse the path
- [ ] One-off re-encode of the existing catalogue (110 prod / 44 staging objects as of 2026-08-19 — the prod count in this item was stale). Mint **new** hash-named paths and update `products.image_path` — a bucket URL's bytes are immutable by contract (the `minimumCacheTTL` justification in `next.config.ts`), so never rewrite an object in place. Hashing here means encoding 44 prod images instead of 104, and the dedup falls out of the same migration; delete the freed copies at the end. (The orphans found in the 2026-08-19 audit — 6 prod, 30 staging — were already deleted that day as a one-off; both buckets now hold only referenced objects: 104 prod / 15 staging)
