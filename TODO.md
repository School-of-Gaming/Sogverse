...
### Locations: one shared tree component; creation restricted to sites

Admins never hand-type a place name: every level above a `site` arrives by seed or by materializing an entry from an official catalog (see `src/services/locations/CLAUDE.md`). The standalone `/admin/locations` CRUD page was **deleted**, and all surfaces share one presentational component — `src/components/locations/location-tree.tsx` (single-select for the product picker, multi-select for gedu coverage; data + create handler injected, demoed in `/admin/ui-components`). The product location picker is the only place locations are written. Follow-ups:

- [ ] **`useUpdateLocation` + the `PATCH /api/admin/locations/[id]` route are now unused.** Renaming locations was only reachable from the deleted admin page; the route + hook (`src/services/locations/`) are dead, as is `LocationFormDialog`'s edit mode. Remove them, or repurpose if we add a site-rename affordance to the picker.
- [ ] **Consider enforcing site-only creation server-side.** `POST /api/admin/locations/create` and `createLocationBody` (`src/services/locations/locations.contracts.ts`) still accept any `location_type` — the restriction is UI-only, and the materialize route is now the sanctioned path for everything above a site. A scripted admin call could still create a region/municipality by hand. Tighten the contract to `type === 'site'` if we want the invariant enforced at the API.
- [ ] **Parent home location (future feature) — persistence decision made, not built.** When parents get to provide their location, the picker browses the public static catalogs (works for any commune, no admin involvement, no new access control). Persist the choice as a catalog reference (`country_code` + `external_code`) on the profile — a parent's home location is profile data, not shared reference data, so no `locations` row and no write-path loosening. Only if some table must one day FK a parent's location does a self-scoped materialize route become necessary.
- [ ] **Dead admin-page-only i18n keys.** A handful of `admin.locations.*` keys (title, description, searchPlaceholder, noLocationsYet, noLocationsMatchSearch) were only used by the deleted page; the dialog still uses the rest of the namespace. `noLocationsYet` is also now wrong ("Add a country to get started" — you cannot). Prune them across all five `messages/*.json` when convenient.
